...
\| v \|_{ h }^{ 2 }  &= \| v \|_{ a_{h},* }^{ 2 } = \| v \|_{ a_{h} }^{ 2 }  + \| h^{\frac{1}{2}} \left\{ \partial _{nn } v\right\}  \|_{ \mathcal{F} _{h} }^{  } \quad  v\in V \oplus V_{h}
    \end{split}
\]
Suppose we have the C0IP problem described in \eqref{eq:C0IP_A_F} with the notation given in \eqref{eq:C0IP_A_h}. Then is the coercivity be computed such that
\[
    \begin{split}
...
\quad \textit{Cauchy-Scwartz inequality} \quad
