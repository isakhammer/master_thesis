...
\end{split}
.\end{equation}
We will assume for now that $u \in H^{4}\left( \Omega  \right) $, $\gamma $ is nonnegative constants and $f \in L_{2}\left( \Omega  \right) $. We may consider the functions $g_{1}$ and $g_{2}$ as time independent boundary requirements. Such problems as \eqref{eq:bi_problem} are often associated with the Cahn-Hilliard model
...
\begin{equation*}
 \int_{\Omega }^{} f dx = \int_{\partial \Omega }^{} g_{2} ds $ if $\gamma  = 0.
.\end{equation*}
This can easily be seen using the substitution $v=1$ in \eqref{eq:bi_weak1}. Therefore, can we let the extended solution space \[
V^{*} = \begin{cases}
    V \quad & \gamma > 0 \\
...
\right>_{E} \\
& = \sum_{T \in  \mathcal{T} _{h}}^{} \left( D^2u, D^2v \right)_{T}  + \sum_{E \in
\mathcal{F} ^{ext}_{}}^{} \left<q, v  \right> _{E}
+ \sum_{E \in \mathcal{F}  ^{int}}^{} \left<\partial _{nn} u , \jump{ \partial _{n} v }
\right>_{E} \\
\end{split}
\end{equation}
