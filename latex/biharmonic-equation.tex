...
Where the interior edges is computed in this way
\begin{equation*}
    \begin{split}
        (I) &  = \sum_{E \in \mathcal{F}_{h}^{int} }^{}   \left<\partial _{n^{+}} \Delta u^{+} ,v^{+}\right>_{E} +
        \left<\partial _{n^{-}} \Delta u^{+} ,v^{-}\right>_{E} =   \sum_{E \in \mathcal{F}_{h}^{int} }^{} \int_{E}^{}
        \jump{ \partial _{n} \Delta u \cdot v } =
        \sum_{E \in \mathcal{F}_{h}^{int} }^{} \int_{E}^{}
         \mean{ \partial _{n} \Delta u } \underbrace{\jump{ v }}_{= 0}    + \underbrace{\jump{ \partial _{n} \Delta u
         }}_{= 0}    \mean{ v } \\
        (II) &  = \sum_{E \in \mathcal{F}_{h}^{int} }^{}    \left<\partial _{n^{+}t} u^{+}, \partial_{t} v^{+}
        \right>_{E} +  \left<\partial _{n^{-}t} u^{-}, \partial_{t} v^{-}
\right>_{E}    =   \sum_{E \in \mathcal{F}_{h}^{int} }^{} \int_{E}^{}
        ...
        (III) &  = \sum_{E \in \mathcal{F}_{h}^{int} }^{}    \left<\partial _{n^{+}n^{+}} u^{+}, \partial_{n^{+}} v^{+} \right>_{E} +  \left<\partial _{n^{-}n^{-}} u^{-}, \partial_{n^{-}} v^{-} \right>_{E}    =   \sum_{E \in \mathcal{F}_{h}^{int} }^{} \int_{E}^{} \jump{ \partial _{nn} u \cdot  \partial_{n} v } = \sum_{E \in \mathcal{F}_{h}^{int} }^{} \int_{E}^{} \mean{ \partial _{nn} u    } \underbrace{\jump{ \partial_{t} v }  }_{!= 0}    + \underbrace{\jump{ \partial
                 _{nt}  u
