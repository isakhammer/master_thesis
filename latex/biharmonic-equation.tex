...
set of external edges $\mathcal{F}^{ext} _{h}$, which is all edges along $\partial \Omega $, and the interior edges $\mathcal{F} ^{int}_{h}$. 
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/mesh.jpg}
    \caption{Example of $\Omega \subset \mathbb{R} ^{2}$ which includes $\mathcal{T} _{h}$ and $ \mathcal{F}_{h}$   }
