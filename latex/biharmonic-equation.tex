...
but we don't have boundary condition for $\Delta u$. \todo{ why no b.c. for $\Delta u$ ? } Instead, we can formulate the problem so
...
where $D^2u:D^2v$ is the inner product and similarly for $\partial _{nn} u = n\cdot D^2 u \cdot n$ .

We can now formulate the full weak formulation by solving for a $u \in  V$ such that
\begin{equation}
    \label{eq:bi_weak1}
...
\end{cases}
\]
Thus, the unique solution in $v \in V^{*}$ belongs to $H^{3 }\left( \Omega  \right) $ \todo{why does it belong to $H^3(\Omega )$ } and we get the following
