...
Let the energy norms be on the form,
\begin{equation}
...
Now, let the approximation of the global error function have the form $u - C_{h}u$.
...
\| u - u_{h} \|_{ a_{h,*} }^{  }  & \le \left( 1 + D \widetilde{C} /\hat{\alpha } \right)\cdot \inf_{v_{h} \in  V_{h}}   \| u - v_{h} \|_{ a_{h,*} }^{  }
    \end{split}
\]
By combing Cléments lemma and Céa's lemma can we now, in fact, conclude that we have the convergence rate estimate,
\begin{equation}
