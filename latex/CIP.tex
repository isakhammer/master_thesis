...
Now, using the energy norm defined in \eqref{eq:A_energy_norm} will we be able to find a upper bound of convergence. A important tool in the process the Clement lemma from can we apply the Cléments interpolation operator $C_{h}$, for more detailed
information please investigate \cite{ern04}. Now let the approximation of the global error function have the form $u - C_{h}u_{h}$. Hence
...
& + \alpha \underbrace{\| u - C_{h}u_{h} \|_{\Omega   }^{2}}_{III}  + \underbrace{\| h^{\frac{1}{2}} \mean{ \partial _{nn}\left(  u - C_{h}u_{h}\right)  }   \|_{\mathcal{F} _{h}  }^{ 2 }}_{IV}
    \end{split}
\]
...
sharing at least one vertex with $F \in  \mathcal{F} _{h}$. Furthermore, we also introduce the notation $\partial T$ for integration along the facets for a triangle $T$. This is important for simplifying jump and mean terms, respectively $II$ and $IV$, since \[
    \begin{split}
...
\sum_{F \in \mathcal{F}_{h} }^{}  \| \mean{ v }   \|_{F  }^{  } & =\sum_{F \in \mathcal{F}_{h} }^{} \frac{1}{2} \|  v^{+} + v^{-}    \|_{F  }^{  } \le \sum_{F \in \mathcal{F}_{h} }^{} \frac{1}{2} \| v^{+} \|_{  F}^{  }  + \| v^{-} \|_{F }^{  } \le \sum_{T \in
\mathcal{T}_{h} }^{} \frac{1}{2}\| v \|_{ \partial T }^{  } \\
    \end{split}
\]
...
I & \le  \| D^2\left( u - C_{h}u_{h} \right)  \|_{ \mathcal{T} _{h} }^{ 2 } = \sum_{T \in \mathcal{T} _{h}}^{} \| D^2 \left( u - C_{h}u_{h} \right)  \|_{ T }^{ 2 } \\
 ...
 III  &\le  \alpha  \sum_{T \in \mathcal{T} _{h}}^{}  \| u - C_{h} \|_{ T }^{ 2 } \le  \sum_{T \in \mathcal{T} _{h}}^{} h^{2l} \| u \|_{ \omega \left( T \right)  }^{ 2 } \\
IV & \le \sum_{T \in \mathcal{T} _{h}}^{}  h \| \partial _{nn} \left( u - C_{h} \right)  \|_{\partial T  }^{  } \le  \sum_{T \in  \mathcal{T} _{h}}^{} h \left( h^{l -2 -\frac{1}{2}} \| u \|_{ l, \omega \left( F \right)  }^{  }  \right)
 \end{split}
\end{equation*}
thus
\[
I + II + III +  IV  \le  h^{2\left( l-2 \right) } \| u \|_{ H^{l}\left( \Omega  \right)  }^{  }
\]
