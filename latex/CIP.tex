\section{Continous Interior Penalty Method}%
...
Let $\mathcal{T}  = \left\{ T \right\} $ be a triangulation of $\Omega \subset   \mathbb{R} ^2 $ consisting of triangles $T$. We may also define the set of all edges $\mathcal{F}_{h}$, where every edge is denoted by $E \in \mathcal{F} _{h}$. However, we will distinguish between the
...
A parameter whicch is useful is the maximum diameter $h$ of the set of triangles $\left\{ T \right\} $, which we to be defined s.t.,
...
where the $diam( T )$ is the largest edge for a triangle $T$. We will also assume mesh conform i.e., if $T_{1} \neq T_{2 }$  and $T \cap T_{2} \neq \emptyset  $ , then they share either a vertex or a edge.
