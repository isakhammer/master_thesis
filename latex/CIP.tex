...
\begin{figure}[htpb]
    ...
    \caption{ A plot of the manufactured solution \eqref{eq:man_sol} on $\Omega = (0, 2 \pi ) ^2$  }
    \label{fig:man_sol}
\end{figure}
...
numerical experiments was done on the mesh discretization illustrated in figure \ref{fig:main} for different diameters $h$. Now let $e = u - u_{h}$. We want to show that $\| e \|_{ L_{2}\left( \Omega  \right)  }^{  } \le  C  h^{p_{1}} $ and $\| e \|_{
H^{1}\left( \Omega  \right)  }  \le  C h^{p_{2}}$
...
\caption{Illustration of mesh discretization on the unit square $\Omega = (0,2 \pi)^{2} \subseteq  \mathbb{R} ^{2}$
    with triangulation $\mathcal{T} _{h}$. The chosen instances had the diameters $h = \left\{ 2\pi  /10, 2\pi /20,2\pi /40 \right\}$ in figures \ref{main:a},
    \ref{main:b} and \ref{main:c}.
}
...
\end{minipage}\par\medskip
\centering
\subfloat[]{\label{main:c}\includegraphics[scale=.05]{figures/convergence/convergence_d_4.png}}

% \subfloat[]{\label{main:c}\includegraphics[scale=.03]{figures/convergence/convergence_d_2.png}}

\caption{ Convergence plots for $\| u - u_{h} \|_{ L_{2}\left( \Omega  \right)  }^{  } $ and $\| u - u_{h} \|_{ H^{1}\left( \Omega  \right)  }^{  } $  using lagrangian orders $\mathcal{P}_{k} $  .}
\label{fig:man_conv}
\end{figure}
