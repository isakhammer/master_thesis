...
}^{ 2 } $ and then, according to Lax Milgram the problem is well posed.
...
\end{cases}
\]
Let $u_{h} \in \mathcal{P} ^{k}$ and $u \in  H^{l}$, then we assume $3 \le  l \le k+1$.

We want to compute the expected convergence rate of the energy norm.
Now, using the energy norm defined in \eqref{eq:A_energy_norm} will we be able to find a upper bound of convergence.


A important tool in the process we will apply is the Cléments interpolation operator $C_{h}$ used for interpolation on non smooth
functions which is defined as a local $L^{2}$ projections onto the so-called macroelements, that is, $C_{h}: H^{m} \left( \Omega  \right) \mapsto V_{h}$. For further detailed information please investigate \cite{ern04}.
...
Now, let the approximation of the global error function have the form $u - C_{h}u_{h}$. Hence,
the energy norm error is formulated as, \[
    \begin{split}
...
We denote a patch $\omega \left( T \right) $ as the set of elements in $\mathcal{T} _{h}$  sharing at least one vertex with $T \in \mathcal{T} _{h}$ . And similarly we denote $\omega \left( F \right) $ the set of all elements in $\mathcal{T}_{h} $
sharing at least one vertex with $F \in  \mathcal{F} _{h}$.
Furthermore, we also introduce the notation $\partial T$ for integration along the facets for a triangle $T$. This is important for simplifying jump and mean terms, respectively term $II$ and
$IV$, since this property arise, \[
    \begin{split}
...
I & \le  \| D^2\left( u - C_{h}u_{h} \right)  \|_{ \mathcal{T} _{h} }^{ 2 } = \sum_{T \in \mathcal{T} _{h}}^{} \| D^2 \left( u - C_{h}u_{h} \right)  \|_{ T }^{ 2 } \\
 & \le \sum_{T \in \mathcal{T} _{h}}^{}  \| \left( u - C_{h}u_{h} \right)  \|_{2,T  }^{  } \le  \sum_{T \in \mathcal{T} _{h}}^{}  h_{T}^{2\left( l-2 \right) } \| u \|_{l, \omega \left( T \right)   }^{  } \\
    ...
    \right)^{2}  \\
    & \le \sum_{T \in \mathcal{T} _{h}}^{}  h^{2(l-2)} \| u \|_{ l, \omega \left( F \right)  }^{ 2 } \\
 III  &\le  \alpha^2  \sum_{T \in \mathcal{T} _{h}}^{}  \| u - C_{h} \|_{ T }^{ 2 } \le  \sum_{T \in \mathcal{T} _{h}}^{} h^{2l} \| u \|_{ \omega \left( T \right)  }^{ 2 } \\
IV & \le \sum_{T \in \mathcal{T} _{h}}^{}  h \| \partial _{nn} \left( u - C_{h} \right)  \|_{\partial T  }^{2  } \le  \sum_{T \in  \mathcal{T} _{h}}^{} h \left( h^{l -2 -\frac{1}{2}} \| u \|_{ l, \omega \left( F \right)  }^{  }  \right)^{2} \le
\sum_{T  \in  \mathcal{T} _{h}}^{} h^{2(l-2)}  \| u \|_{ l, \omega \left( F \right)  }^{ 2 }
 \end{split}
\end{equation*}

By gathering all terms can we observe the results,

\[
    \begin{split}
        I + II + III +  IV  &  \le  \sum_{T \in \mathcal{T} _{h}}^{}  h_{T}^{2\left( l-2 \right) } \| u \|_{l, \omega \left( T \right)   }^{  } +   2\cdot h^{2(l-2)}  \| u \|_{ l, \omega \left( F \right)  }^{ 2 } + h_{T}^{2l} \| u \|_{l, \omega \left( T \right)}
        \\
        &  \le  h^{2\left( l-2 \right) } \| u \|_{ H^{l}\left( \Omega  \right)  }^{2  }.
    \end{split}
\]

Hence, we get the final convergence rate estimate,
\[
\| u - C_{h}u_{h} \|_{  }^{  } \le  h^{l(l-1)} \| u \|_{H^{l} \left( \Omega  \right)  }^{  }.
\]
