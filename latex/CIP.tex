...
\[
\left( \nabla \left( \nabla ^2 w \right), \nabla  v  \right)_{T} = \sum_{i=1}^{2}  \int_{T}^{} \left( \nabla \nabla w_{x_{i}} \right)\cdot  v_{x_{i}} \ dx   =  \int_{T}^{} D^2w: D^2v \ dx - \int_{\partial T}^{} \left( \partial _{n} \nabla w \right) \cdot  \nabla v  \ ds
\]
...
\]
 Thus, letting $u,v \in
H^{4}\left( T  \right) $ and using \eqref{eq:weak_form_identity}  does this hold ,
...
\quad \textit{ Youngs epsilon inequality} \quad
  ...
  \mathcal{F} _{h} }^{  } \ge C \| v_{h} \|_{ a_{h} }^{  2}   \text{ where }
    \end{split}
\]
This is true if $C=\min\left\{  \alpha , 1 /2\right\}$.
