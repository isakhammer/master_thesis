...
Generally speaking, for both examples do we observe that the convergence rate is robust for most choices of $\gamma$ for any order. However, we can also observe in both cases we see numerical instabilities for order $k=4$ and $h=\frac{1}{128}$. The cause is
...
Observing the convergence rate can we see in both examples that the theoretical estimate for the energy norm,
\[
\| u - u_{h} \|_{h}^{  } \lesssim O\left( h^{k-1} \right),
\]
matches the observed EOC.
...
\| u - u_{h} \|_{ \Omega  }^{  }&  \lesssim O\left( h^{k+1} \right) , \quad k\ge  3
    \end{split}
\]
As predicted does the EOC, in fact, match with the estimates including some numerical artifacts.
Lastly, while we cannot provide a theoretical estimate for $H^{1}$ norm, do we see that the estimate, \[
\| u -u_{h} \|_{  H^{1}\left( \Omega  \right)  }^{  }   \lesssim O\left( h^{k} \right), k\ge 3 ,
\]
