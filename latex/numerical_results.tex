...
All numerical experiments were carries out on the unit square with length $L$. The convergence analysis were done for polynomial basis, $\mathcal{P}_{k} $, with order $k=2,3,4$ and a triangulation $\mathcal{T} _{h}$ for $\frac{h}{L} = \frac{1}{4}, \frac{1}{16}, \frac{1}{32} \frac{1}{64}, \frac{1}{128} $.
...
parametrization of \eqref{eq:man_sol}. Each examples consists of a convergence plot of different penalty parameters $\gamma$ of the $L^{2}$ norm for each order and a more extensive analysis by comparing the convergence rate of $L^{2}$ , $H^{1}$ and
energy norms. A convenient tool to measure convergence is by computing the experimentally observed convergence rate (EOC), which is defined as follows,
\[
EOC(i+1) = \frac{\log\left( \frac{\| e\left( h_{i+1} \right)  \|_{ \mathcal{V}  }^{  }}{\| e\left( h_{i} \right) \|_{\mathcal{V}   }^{  } }   \right)}{\log\left( \frac{h_{i+1}}{h_{i}}  \right) }   .
\]
