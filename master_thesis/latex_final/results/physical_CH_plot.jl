...
function main()
    ...
    p4 = plot(its_circle,  data_circle.E1s, size=default_size,   xscale=:log10, legend=false, ylabel=L"$E_1^m$", xlabel=L"$m$", linecolor = :red)
    plot!(p4, its_flower,  data_flower.E1s, xscale=:log10,        linecolor = :blue)
    # scatter!(p4, its_circle, data_circle.E1s, markersize=2, markeralpha=0.5, markercolor=:red)
    # scatter!(p4, its_flower, data_flower.E1s, markersize=2, markeralpha=0.5, markercolor=:blue)

    p5 = plot(its_circle, data_circle.E2s, size=default_size, label = "Circle", xscale=:log10, ylabel=L"$E_2^m$", xlabel=L"$m$", linecolor = :red)
    plot!(p5, its_flower, data_flower.E2s, xscale=:log10, label = "Flower", linecolor = :blue)
    # scatter!(p5, its_circle, data_circle.E2s, markersize=2, markeralpha=0.5, markercolor=:red, label=false)
    # scatter!(p5, its_flower, data_flower.E2s, markersize=2, markeralpha=0.5, markercolor=:blue, label=false)

    p6 = plot(its_circle[1:end-1], data_circle.Es[1:end-1] .- data_circle.Es[2:end], size=default_size, xscale=:log10,   xlabel=L"$m$", ylabel=L"$\delta E^m$", legend=false, linecolor = :red)
    plot!(p6, its_flower[1:end-1], data_flower.Es[1:end-1] .- data_flower.Es[2:end], xscale=:log10, yscale=:log10,       linecolor = :blue)
    # scatter!(p6, its_circle[1:end-1], data_circle.Es[1:end-1] .- data_circle.Es[2:end], markersize=2, markeralpha=0.5,markercolor=:red)
    # scatter!(p6, its_flower[1:end-1], data_flower.Es[1:end-1] .- data_flower.Es[2:end],markersize=2, markeralpha=0.5, markercolor=:blue)
