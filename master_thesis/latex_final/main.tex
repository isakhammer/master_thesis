...
Our goal is to construct abstract error estimates for the discrete bilinear form $a_{h}( \cdot ,\cdot ) $.
...
A necessary assumption is that the discrete bilinear form $a_{h}( \cdot ,\cdot ) $  is bounded in $W \otimes V_{h} \times V_{h}$, i.e.
\begin{equation}
    \label{eq:ah_star}
a_{h}( v + v_{h}, w_{h}) \lesssim  \| v + v_{h} \|_{ V,* }^{  } \| w_{h} \|_{V_{h}  }^{  } \quad  \forall v+ v_{h} \in W \otimes V_{h}, w_{h} \in V_{h}
\end{equation}
We also assume that $\| v_{h} \|_{V,*  }^{  } \lesssim  \| v_{h} \|_{ V_{h} }^{  }  \ \forall v_{h } \in  V_{h} $. Hence, we have the estimate \begin{equation}
a_{h}( v_{h}, w_{h}) \lesssim  \|  v_{h} \|_{ V,* }^{  } \| w_{h} \|_{V_{h}  }^{  } \lesssim \|  v_{h} \|_{ V,* }^{  } \| w_{h} \|_{V_{h}  }^{  } \quad  \forall  v_{h},  w_{h} \in V_{h}.
\end{equation}
This, is good since we can now utilize the stability and coercivity of the discrete bilinear form presented in \eqref{eq:discrete_wellposed}.

Since $V_{h} \not \subset V_{h}$, it is clear that we cannot directly employ the standard Cea's lemma \cite[p. 66]{quartdiff}. Yet, we are indeed equipped to formulate an effective workaround.

\begin{lemma}[Nonconform Cea's Lemma]
    Let $u$ be the exact solution. Let $V_{h} \not \subset V$. Assume $a_{h}( \cdot ,\cdot ) $ is well-posed in $V_{h} \times _V_{h}$, the discrete stability \eqref{eq:ah_star} and Galerkin orthogonality \eqref{eq:galerin_orth}. Then, the following error estimate hold,
    \begin{equation}
        \| u - u_{h} \|_{ V_{h},*  }^{  }  \lesssim  \inf_{v_{h} \in  V_{h}}  \| u - v_{h} \|_{ V_{h},*  }^{  }.
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    \end{equation}
\end{proof}
A more general proof, see \cite[Theorem 1.35]{pietro2012}
