...
to til tre uker, og da setter mang en student (les: meg) som setter stor pris på at foreleserne er veldig positive og åpne for å legge opp til et nytt forsøk.
...
Now, in the context of the biharmonic problem. First of all, int the numerical implementation we only considered element of order $k=2$, hence, a good extension would be to look at $k=3,4$. It would also be interesting to compare this method to other
unfitted methods, such at aggregated finite element methods and isogeometric analysis.

Specifically, for the Cahn-Hilliard problem. In this report we restricted ourself to handle a constant mobility $M=1$ and a polynomial approximation of the non-linearity $F(u) =\frac{1}{4} ( u^2 - 1)^{2}  $. However, to adapt this formulation to more
diverse physical problems this has to be taken into account. From the numerical point of view is the method very inefficient in the sense of adaptivity both in space and time. Hence, in future work, would exploiting more advanced adaptive methods be
suitable.
