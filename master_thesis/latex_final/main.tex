...
\begin{abstract}
The fourth order Cahn-Hilliard equation, with its wide range of applications in phase-separation, poses numerical solution challenges due to its non-linearity, stiffness and fourth-order derivatives. This thesis introduces a novel, stabilized,
unfitted, cut continuous interior penalty method specifically designed for the biharmonic problem with Cahn-Hilliard type boundary conditions and then extended to handle the Cahn-Hilliard equation. Our approach combines the theoretical cut
discontinuous Galerkin framework for the Poisson problem, as proposed by \cite{gurkan2019stabilized}, with the continuous interior penalty biharmonic formulation presented by \cite{brenner2012} and \cite{feng2007fully}. We provide proof that this
...
equation using a minimalistiv Implicit-Explicit (IMEX) time discretization scheme to manage the non-linearities.
\end{abstract}

\begin{abstract}

 Cahn-Hilliard-ligningen har et bredt av anvendelser innen fase-separasjon, men gir numeriske utfordringer på grunn av sin ikke-linearitet, stivhet og fjerdeordens deriverte. Denne avhandlingen introduserer en ny og stabilisert cut continuous interior
 penalty method spesifikt utviklet for det biharmoniske problemet med Cahn-Hilliard-liknende randbetingelser, og deretter utvidet for å håndtere Cahn-Hilliard-ligningen. Vi vil anvende det teoretiske rammeverket for discontinuous Galerkin
 Poisson-problemet, slik det er foreslått av \cite{gurkan2019stabilized}, i kombinasjon med continious interioir penalty method for det biharmoniske problemet presentert av \cite{brenner2012} og \cite{feng2007fully}. Vi gir teoretisk grunnlag for at
 denne metoden er veldefinert  og har optimal konvergens. Vi deretter gir validerer de teoretiske resultatene med numeriske eksperimenter. Til slutt demonstrerer vi metoden kan bli utvided å håndtere Cahn-Hilliard-ligningen ved hjelp av en minimalistisk Implisitt-Eksplisitt (IMEX) tidsdiskretiseringsmetode for å håndtere ikke-lineariteter.
