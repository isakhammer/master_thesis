...
\begin{lemma}[Nonconform Cea's Lemma]
    ...
    Assume that the discrete bilinear form $a_{h}( \cdot ,\cdot ) $ is well-posed in $V_{h} \times V_{h}$. Additionally, assume that both the discrete stability (as given by \eqref{eq:ah_star}) and Galerkin orthogonality (as outlined in \eqref{eq:galerin_orth}) are valid
