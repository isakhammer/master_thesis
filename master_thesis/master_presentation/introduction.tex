...
\end{frame}
\begin{frame}
    The following results has been shown to hold for the bilinear form.

    \begin{block}{Well-posedness }
         We say that the discrete bilinear form $a_{h}$ is wellposed on $V_{h}$ if there exists an constant $C > 0 $ s.t. \[
             \begin{split}
    ...
    \begin{block}{Optimal convergence}
    \end{block}
\end{frame}
