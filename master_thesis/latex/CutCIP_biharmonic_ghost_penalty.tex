...
\begin{proof}
  From Lemma \ref{lemma:bi_inv_gh_lemma} is it clear that $
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    Here we added a zero term $v^{e} \in H^{s}( \Omega_{h} ) $  since jump vanishes for the for the first $r-1$ terms, i.e. $\jump{ \partial ^{j}_{n} v^{e} } = 0  \  \forall j \le r-1$.
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    Here we applied the estimate \eqref{eq:bi_projection_estimates_2}.
