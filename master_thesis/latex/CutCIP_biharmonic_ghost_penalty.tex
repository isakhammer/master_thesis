...
Let $  v \in H^{k}( \Omega ) $. The generalization of the normal derivative is denoted as,
\begin{equation}
    \partial _{n}^{k} v = \sum_{   \abs{ \alpha    } =  k}^{} \frac{ \partial^{\alpha } v \  n^{\alpha } }{\alpha !},
\end{equation}
where the component-wise product of the normal vector is $n^{\alpha } = n_{1}^{\alpha _{1}} \ldots n_{d}^{\alpha _{d}}$ and the derivative $\partial ^{\alpha }v $ as in Equation \eqref{eq:der}. Observe that $\partial ^{k}_{n} v = v $, $ \partial ^{1}_{n} v = \nabla v
 \ n $ and $ \partial ^{2}_{n} v = \frac{1}{2} n^{T} D^2 v \ n$. Remark that the factor $\frac{1}{2}$  is often hidden in the $ \lesssim 1$ inequality.
% \footnote{
...
Here we added a zero term $v^{e} \in H^{s}( \Omega_{h} ) $  since jump vanishes for the for the first $r-1$ terms, i.e. $\jump{ \partial ^{j}_{n} v^{e} } = 0  \  \forall s \le r-1$.
    ...
    \begin{equation}
        ...
        & \lesssim \sum_{j=0}^{r-1} h^{2j-3}  h^{2r - 2j - 1}\| v  \|_{r,\Omega  }^{2} \lesssim  \sum_{j=0}^{r-1}   h^{2( r  - 2 )}\| v  \|_{r,\Omega  }^{2}
        \end{split}
    \end{equation}
    Here we applied \eqref{eq:bi_projection_estimates_2}.
