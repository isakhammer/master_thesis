...
\begin{proof}
  From Lemma \ref{lemma:bi_inv_gh_lemma} is it clear that $
    ...
    What we can see is that the first term can easily be estimated using the a priori estimate \eqref{eq:bi_projection_estimates_2} \red{Not sure if this estimate makes sense since we must extend to the full space $\partial \mathcal{T} _{h}$, see \eqref{eq:mean_jump_estimate} },
    \begin{equation}
        \begin{split}
