...
\begin{proof}
  From Lemma \ref{lemma:bi_inv_gh_lemma} is it clear that $
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    \red{Why dont we take inverse inequality directly on $ \sum_{j=0}^{k}  \gamma _{j} h^{2j-3}  \| \jump{ \partial ^{j}_{n} C^{e}_{h} v }  \|_{\mathcal{F}_{h}^{g} }^{2}$? }
...
\red{\textbf{Version 2.} Using the trace inequality \eqref{eq:trace:1}, i.e.,
    \begin{equation}
\|  D ^{j} v     \|_{ \partial T }^{2} \lesssim h^{-\frac{1}{2}}\|  D ^{j} v     \|_{  T }^{2} + h^{\frac{1}{2}}\|  D ^{j+1} v     \|_{  T }^{2}, \quad v \in H^{j+1}( T )
    ...
    \begin{equation}
        ...
        \mathrm{I}  & \lesssim \sum_{j=0}^{r-1} h^{2j-3}   \|  D ^{j}  ( C^{e}_{h} v - v^{e})     \|_{ \partial \mathcal{T}_{h} }^{2} \\
                    & \lesssim \sum_{j=0}^{r-1} h^{2j-3}   h^{-1}( \|  D ^{j} ( C^{e}_{h} v - v^{e})     \|_{  T }^{2} + h \|  D ^{j+1} ( C^{e}_{h} v - v^{e})     \|_{  T }^{2} )
        \end{split}
    \end{equation}
