...
\end{equation}
where the component-wise product of the normal vector is $n^{\alpha } = n_{1}^{\alpha _{1}} \ldots n_{d}^{\alpha _{d}}$ and the derivative $\partial ^{\alpha }v $ is as defined in Equation \eqref{eq:der}. Observe that $\partial ^{0}_{n} v = v $, $ \partial ^{1}_{n} v = \nabla v
 \ n $ and $ \partial ^{2}_{n} v = \frac{1}{2} n^{T} D^2 v \ n$.
% \footnote{
