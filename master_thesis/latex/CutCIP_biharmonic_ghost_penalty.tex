...
%     Let $T_{1}, T_{2} \in  \mathcal{T} _{h} $ share a common facet $F \in \mathcal{F}_{h} $ and let $v_{h} \in  V_{h}$ with polynomial degree $k$. Then does this hold,
%     \begin{equation}
%     \| v_{h} \|_{ T_{1} }^{  2}  \lesssim  \| v_{h} \|_{ T_{2} }^{2  }  + \sum_{j=1}^{k}  h^{2j +1} ( \jump{ \partial _{n}^{j} v_{h}}, \jump{ \partial _{n}^{j} v_{h}}    )_{F}.
%     \end{equation}
