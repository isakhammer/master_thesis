...
\begin{proof}
  From Lemma \ref{lemma:bi_inv_gh_lemma} is it clear that $
    ...
    \begin{equation}
        \label{eq:est_gh:1}
        \begin{split}
        \abs{ C^{e}_{h} v }_{g_{h}}^{2} & = \sum_{j=1}^{k}  \gamma _{j} h^{2j-3}  \| \jump{ \partial ^{j}_{n} C^{e}_{h} v }  \|_{\mathcal{F}_{h}^{g} }^{2} \\
                                        & = \sum_{j=1}^{k}  \gamma _{j} h^{2j-3}  \| \jump{ \partial ^{j}_{n} C^{e}_{h} v }  \|_{\mathcal{F}_{h}^{g} }^{2} \\
...
% &= \sum_{j=r}^{k}  \gamma _{j} h^{2j-3} ( \jump{ \partial ^{j}_{n} v }, \jump{ \partial ^{j}_{n} v }  ) _{\mathcal{F}_{h}^{g} } \\
    ...
    \begin{equation}
        \label{eq:est_gh:2}
        \| \partial _{n}^{j}v_{h} \|_{ F }^{2  } \lesssim \| D^{j}v_{h} \|_{ F }^{2  } \lesssim h^{2( r- j ) - 1}\| D^{r}v_{h} \|_{  T}^{2  } = h^{2( r- j ) - 1} \| v_{h} \|_{r, T  }^{  }   .
    \end{equation}
    Hence, combining \eqref{eq:est_gh:1} and \eqref{eq:est_gh:2} we have \begin{equation}
        \abs{ C^{e}_{h} v }_{g_{h}}^{2} \lesssim \sum_{j=r}^{k}  \gamma _{j} h^{2j-3}
    \end{equation}
