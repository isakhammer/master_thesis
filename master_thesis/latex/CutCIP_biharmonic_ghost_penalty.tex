...
Inserting $w_{h} = \partial ^{j} _{n} v_{h}$, we get $ \| \jump{ \partial _{x_{i}} \partial ^{j}_{n} v_{h} }   \|_{F  }^{2  } = \| \jump{\partial ^{j+1} v \  e_{x_{i}} \cdot n  }   \|_{  }^{  } $

Utilizing Lemma \ref{lemma:bi_local_facet_estimate} is it clear that \begin{equation}
    \| \partial _{x_{i}} v_{h} \|_{  T_{1}}^{2  } \lesssim \| \partial _{x_{i}} v \|_{ T_{2} }^{  2} + \sum_{j=0}^{k}
\end{equation}
