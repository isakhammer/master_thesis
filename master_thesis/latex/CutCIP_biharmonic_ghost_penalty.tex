...
Following exactly the same steps as in \textbf{Estimate \eqref{eq:local_ghost:0}}, but changing out Lemma \ref{lemma:bi_local_facet_estimate} with estimate \eqref{eq:local_ghost:0}, it is easy to conclude the following. For any  $i_{1},i_{2} \in
\left\{ 1,\ldots, d \right\}$ we have,
\begin{equation}
        \| \partial _{x_{i_1} x_{i_2}} v \|_{ T_{1} }^{  }  \lesssim \| \partial_{ x_{i_1} x_{i_2} } v_{h} \|_{ T_{2} }^{  } + \sum_{0\le j\le k}  {h^{2j +1}}^{} ( \jump{ \partial _{n}^{j} \partial _{x_{i_1} x_{i_2}} v_{h} }, \jump{ \partial ^{j}_{n}
        \partial _{x_{i_1} x_{i_2}} v_{h} }    )_{F}
\end{equation}
Using the element-wise argument as in \textbf{Estimate \eqref{eq:local_ghost:1}} for the Hessian and the Laplace operator, we can conclude that \eqref{eq:local_ghost:2} and \eqref{eq:local_ghost:3} hold.
