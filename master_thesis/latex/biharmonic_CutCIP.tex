...
\todo[inline]{ Not sure if \eqref{eq:bi_projection_estimates_3} holds. And why is \eqref{eq:bi_projection_estimates_2} summing over $ T \in \mathcal{T} _{h}$ and not $ F \in  \mathcal{F} _{h}$.      }
...
\begin{proof}
    ...
    And for the boundary term, \[
        ...
        \end{split}
    \]
    \todo[inline]{ Not able to get rid of $\partial _{n}$, but still be able to integrate over $\mathcal{F}_{h}$.  }
    We know that $\| \mean{ u }   \|_{ \mathcal{F} _{h} }^{  }  \lesssim  \| u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $ and that $\| \partial _{n} u \|_{ F  }^{} \lesssim h^{-\frac{1}{2}} \| \nabla u \|_{ \partial T }^{  }  $.
