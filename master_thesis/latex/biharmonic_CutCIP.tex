...
\begin{proof}
    ...
    \[
    \begin{split}
       a_{h}( v,v) &=   \|\alpha v  \|_{\Omega   }^{2} + \| D^2v \|_{ \mathcal{F} ^{int}_{h} \cap \Omega  }^{2  } + 2 ( \mean{ \partial _{nn} v }, \jump{ \partial _{n} v }    )_{\mathcal{F} ^{int}_{h} \cap \Omega }  + 2 (  \partial _{nn} v ,
    ...
    Using inverse-inequalities \eqref{eq:bi_cut_inverse_1} and \eqref{eq:bi_cut_inverse_2} and the Corollary \ref{cor:bi_inverse_thm} can we easily observe that \[
        \begin{split}
     \| \mean{ \partial _{nn}v } \|_{ \mathcal{T}_{h} \cap \Omega    }^{  2} & \le C_{I} \| D^2 v \|_{ \mathcal{T}_{h}   }^{2  } \le  C  (\| D^2 v \|_{ \mathcal{T} _{h} }^{ 2 }  + \| v \|_{ g_{h} }^{2  } )  \\
     \|  \partial _{nn}v  \|_{ \Gamma     }^{ 2 } & \le C_{I} \| D^2 v \|_{ \mathcal{T} _{h}  }^{2  } \le C  (\| D^2 v \|_{ \mathcal{T} _{h} }^{ 2 }  + \| v \|_{ g_{h} }^{2  } )
        \end{split}
    \]
    Thus, by applying Youngs $\varepsilon $-inequality, $2ab \le  \varepsilon^{-1} a^{2} + \varepsilon b^{2} $, is it natural to see that,
    \[
        \begin{split}
- C_{I}^{\frac{1}{2}} \| D^2 v    \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{  }  \|h^{-\frac{1}{2}} \jump{ \partial _{n} v }   \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{  }
& \ge - \frac{1}{\varepsilon } C_{1}\|  D^2 v  \|_{ \mathcal{T} _{h}   }^{2  } -  \varepsilon \|h^{-\frac{1}{2}} \jump{ \partial _{n} v }   \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{2  } \\
- C_{I}^{\frac{1}{2}} \| D^2 v    \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{  }  \|h^{-\frac{1}{2}} \jump{ \partial _{n} v }   \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{  }
& \ge - \frac{1}{\varepsilon } C_{I}\|  D^2 v  \|_{ \mathcal{T} _{h}   }^{2  } -  \varepsilon \|h^{-\frac{1}{2}} \jump{ \partial _{n} v }   \|_{ \mathcal{F}^{int}_{h}\cap \Omega   }^{2  } \\
        \end{split}
    \]
