...
\begin{proof}
    ...
    \[
    \begin{split}
       a_{h}( v,v) &=   \|\alpha v  \|_{ \mathcal{T}_{h}\cap  \Omega   }^{2} + \| D^2v \|_{ \mathcal{T}_{h}\cap  \Omega  }^{2  } + 2 ( \mean{ \partial _{nn} v }, \jump{ \partial _{n} v }    )_{\mathcal{F} ^{int}_{h} \cap \Omega }  + 2 (  \partial _{nn} v ,
...
& \ge - \frac{1}{\varepsilon } C  (\| D^2 v \|_{ \mathcal{T} _{h} }^{ 2 }  + \abs{ v } _{ g_{h} }^{2  } ) -  \varepsilon \|h^{-\frac{1}{2}}  \partial _{n} v    \|_{ \Gamma    }^{2  } \\
    ...
    \end{split}
    \]
    Hence, setting $\varepsilon = 8C$ and $\gamma = \frac{33}{2} C$ we simplify the problem to \[
        \begin{split}
