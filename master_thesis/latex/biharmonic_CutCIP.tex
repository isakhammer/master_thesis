    ...
    Setting $\varepsilon = 8C$, $\gamma = \frac{33}{2} C$ and $C=1$  we simplify the problem to \[
        \begin{split}
           A_{h}( v,v)  \ge& \   \| \ |\alpha|^{\frac{1}{2}} \    v  \|_{  \Omega   }^{2} + \frac{1}{2}  (\| D^2 v \|_{ \Omega  }^{ 2 }  + \abs{ v } _{ g_{h} }^{2  } )  \\
                       & + \frac{1}{2}\left( \|h^{-\frac{1}{2}}  \jump{ \partial _{n} v }\|_{\mathcal{F} _{h}^{}\cap \Omega   }^{ 2 } + \| h^{-\frac{1}{2}} \partial _{n} v \|_{ \Gamma  }^{ 2} \right) \\
...
Summation over the intersected triangles $\mathcal{T} _{\Gamma }$ implies,
    ...
    \item We will simply start by replacing $v$  by $D^2 v$ and use the Proposition \ref{prop:hessian_change}.
        ...
        Let us denote the tangential operator $P_{F} := I - n_{} \oplus n_{} $, where $n_{}\oplus n_{}$ is an outer product of the normals on the facet $F$. We apply this so we can to decompose the Hessian evaluated on the facet  s.t. \[
