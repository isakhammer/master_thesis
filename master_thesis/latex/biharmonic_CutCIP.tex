...
\begin{proof}
    ...
    \end{split}
    \]
    We will first focus on the symmetry terms.
    Using Cauchy-Schwarz and the inequality can we observe that \[
        \begin{split}
...
& \ge - \frac{1}{\varepsilon } C  (\| D^2 v \|_{ \mathcal{T} _{h} }^{ 2 }  + \abs{ v } _{ g_{h} }^{2  } ) -  \varepsilon \|h^{-\frac{1}{2}}  \partial _{n} v    \|_{ \Gamma    }^{2  } \\
    ...
    \begin{split}
       a_{h}( v,v)  \ge&    \|\alpha v  \|_{ \mathcal{T}_{h}\cap  \Omega   }^{2} + \| D^2v \|_{ \mathcal{T}_{h}\cap  \Omega  }^{2  }  - \frac{1}{\varepsilon } 4C  (\| D^2 v \|_{ \mathcal{T} _{h} }^{ 2 }  + \abs{ v } _{ g_{h} }^{2  } )  \\
                   & + (\gamma - 2\varepsilon  )    \|h^{-\frac{1}{2}}  \jump{ \partial _{n} v }\|_{\mathcal{F} _{h}^{int}  }^{ 2 } + (\gamma - 2\varepsilon  ) \| h^{-\frac{1}{2}} \partial _{n} v \|_{ \Gamma  }^{ 2 }
    \end{split}
    \]


    \todo[inline]{ TODO fix $\| \cdot  \|_{ g_{h} }^{  } $   }
\end{proof}
