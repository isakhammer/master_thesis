...
Recall the notation for the derivates, \[
\partial ^{\alpha} v = \frac{\partial ^{ \alpha_{1}  }  } {\partial^{} x_{1}^{\alpha _{1}}  } \ldots \frac{\partial ^{ \alpha_{d}  }  } {\partial^{} x_{d}^{\alpha _{d}}  } v
\]
Let us introduce the notation \[
\partial _{n}^{j} v = \sum_{\abs{ \alpha  } =1 }^{} \frac{\partial ^{\alpha }v( x) n^{\alpha }}{\alpha !}, \quad \abs{ \alpha  } = \sum_{j}^{d} \alpha_{i}
\]
...
\begin{proof}
    ...
    \item We will simply start by replacing $v$  by $D^2 v$, that it,
        \[
                    \| D^2 v \|_{ \mathcal{T} _{h} }^{2  } \lesssim \| D^2 v \|_{ \Omega  }^{ 2 }  + \sum_{j=0}^{k} h^{2j+1} ( \jump{ D^2  \partial ^{j}_{n} v }, \jump{ D^2  \partial ^{j}_{n} v}    )_{\mathcal{F}_{h}^{g}}
        \]
