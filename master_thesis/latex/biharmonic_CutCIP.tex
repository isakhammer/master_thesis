...
\todo[inline]{ The physical space $\Omega $ is a subset of $\Omega _{e}$, so I do not understand why $\| v^{e} \|_{ m,q,\Omega ^{e}  }^{  } \lesssim \| v \|_{ m,q, \Omega  }^{  } $ should hold.}
...
\todo[inline]{ Not sure if \eqref{eq:bi_projection_estimates_3} holds for smooth boundaries, $\Gamma \in  C^2 $ . And why is \eqref{eq:bi_projection_estimates_2} summing over $ T \in \mathcal{T} _{h}$ and not $ F \in  \mathcal{F} _{h}$.      }
...
\item
    ...
    \]
\item
        Recall $\| \jump{ u }   \|_{ \mathcal{F} _{h} }^{  }  \lesssim  \| u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $ and the inverse estimate $\| \partial _{n} u \|_{ F  }^{} \lesssim h^{-\frac{1}{2}} \| \nabla u \|_{ T }^{  }  $.
     Hence, \[
        \begin{split}
            \gamma \| h^{-\frac{1}{2}} \jump{ \partial _{n} ( u - C_{h}^{e}u ) }   \|_{ \mathcal{F}_{h} \cap \Omega   }^{  2}   &\lesssim \| h^{-\frac{1}{2}}  \partial _{n} ( u - C_{h}^{e}u )    \|_{  \mathcal{F} _{h} }^{2  }
       \lesssim h^{-2} \|   \nabla  ( u - C_{h}^{e}u )    \|_{ \mathcal{F}_{h} }^{2  }     \\
                                                                                                                 &\lesssim h^{-2} \|      u - C_{h}^{e}u     \|_{1, \mathcal{F}_{h} }^{2  }
...
\item
    And for the boundary term, \[
        \begin{split}
            \gamma \| h^{-\frac{1}{2}}  \partial _{n} ( u - C_{h}^{e}u ) \|_{ \Gamma    }^{  2} \lesssim h^{-2} \|   \nabla  ( u - C_{h}^{e}u )    \|_{ \mathcal{T}_{h} }^{2  } \lesssim  h^{-2} \|    u - C_{h}^{e}u     \|_{1, \mathcal{T}_{h} }^{2  }
        \end{split}
    \]
\item

            We also know that $\| \mean{ u }   \|_{ \mathcal{F} _{h} }^{  }  \lesssim  \| u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $ and the $\| \partial _{nn} u \|_{ F  }^{} \lesssim h^{-\frac{1}{2}} \| D^2 u \|_{ T }^{  }  $. It is clear
 ...
 \| h^{\frac{1}{2}} \mean{ \partial _{nn} (u - C_{h}^{e}u) }   \|_{\mathcal{F} _{h}^{int} \cap \Omega   }^{  2} &  \lesssim h^{} \|   \partial _{nn} (u - C_{h}^{e}u)    \|_{\partial \mathcal{T} _{h}   }^{  2}  \lesssim  \|   D^2 (u - C_{h}^{e}u)    \|_{ \mathcal{T} _{h}   }^{  2} \\
            ...
            \]
            \item
                Then as well using \eqref{eq:bi_projection_estimates_3} we get,  \[
              \| h^{\frac{1}{2}} \partial _{nn}(u - C_{h}^{e}u)     \|_{ \Gamma }^{  2} \lesssim   \|  D^2(u - C_{h}^{e}u)     \|_{ \Gamma  }^{  2}
            \]
            \todo[inline]{ Not sure if inverse inequality holds on $\Gamma $  }

    \end{enumerate}
\end{proof}


\todo[inline]{TODO: Cleanup of consistency when using parameters $k,q,r,p,l,s$. My head is exploding of what is what.}
...
\begin{proof}
\todo[inline]{ TODO: Finish proof and update proposition }
\end{proof}
