...
\left\{ 1,\ldots,N \right\} \to \left\{ 1, \ldots, n_{cf} \right\}  $ s.t. $j( i) $ is the index associated with the reference configuration $\hat{A}_{j(i) }$ for corresponding macroelement $A_{i}$. Let us define a $C^{0}$-diffeomorphism $G_{A_{i}}$
from $\hat{A}_{j( i) }$ to $A_{i}$ s.t. for all $\hat{T} \in \hat{A}_{j( i) } $ is the restriction $G_{A_{i}  \mid \hat{T}}$ affine. The Cléments interpolation operator $C_{h}$ is defined as an $L^2$-projection onto the macroelements. That is, given
a reference macroelement $\hat{A}_{j( i) }$ and a function $\hat{v} \in L^{1}( \hat{A}_{j( i) })  $, then $\hat{C}_{j( i) } \hat{v}$  is the unique polynomial in $\mathcal{P}^{k} ( \hat{A}_{j( i) })  $ s.t. \[
\int_{  \hat{A}_{j( i) }}^{} ( \hat{C}_{j( i) } \hat{v} - \hat{v}) p \ dx  = 0 \quad  \forall p \in \mathcal{P}^{k} ( \hat{A}_{j( i) })
\]
Finally, we define the Cléments interpolator $C_{h} : L^{1}( \Omega )  \to \mathcal{P} ^{k}_{c}(\Omega  ) $ s.t.
\[
C_{h} v = \sum_{i=1}^{N} \hat{C}_{j( i) } ( v (G_{A_{i}}) (G^{-1}_{A_{i}}(a_{i})) )\phi _{i},
\]
...
For further detailed information, please investigate \cite[Lemma 1.127]{ern04}.
