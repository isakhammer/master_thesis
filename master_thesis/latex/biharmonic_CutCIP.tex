...
Summation over the intersected triangles $\mathcal{T} _{\Gamma }$ implies,
    ...
    \item We will simply start by replacing $v$  by $D^2 v$ and use the Proposition \ref{prop:hessian_change}.
        ...
        Let us denote the tangential operator $P_{F} := I - n_{F} \oplus n_{F} $, where $n_{F}\oplus n_{F}$ is an outer product for the facet $n_{F}$. We apply this so we can to decompose the Hessian s.t. \[
