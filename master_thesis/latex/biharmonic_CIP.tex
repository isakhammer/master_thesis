...
regularity, and the test function $v$ defined via $V_{h} = \left\{ \mathcal{P} _{c}^{k}( \mathcal{T} _{h}) \text{ or } \mathcal{Q} _{c}^{k}( \mathcal{T} _{h})    \right\}$ in \eqref{def:Vh_background}.
This asserts that when we replace $u \in V_{h}$, the system remains consistent. However, due to the nonconformal nature of $V_{h}$, it becomes necessary to introduce penalty terms to ensure the system is well-posed.
\begin{lemma}
...
We define the two relevant CIP formulations for the \eqref{eq:bi_problem} as follows. The general idea is to construct a bilinear problem $a_{h}: V_{h} \times  V_{h} = \mathbb{R} $ and $l_{h}: V_{h} \to \mathbb{R} $ s.t. there exists an $u \in V_{h}$ s.t. $a_{h}( u_{h}, v_{h}) = l_{h}( v) $ for all $v \in V_{h}$. For the CIP
