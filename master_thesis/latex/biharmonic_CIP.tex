...
\end{equation}
However, by integrating over exterior facets $\mathcal{F} _{h}^{ext}$ and interior facets $\mathcal{F} _{h}^{int}$ we will get e more suitable formulation wichi makes it easier to penalize.

% Recall that any jump over a interior facet $F \subset \mathcal{F} _{h}^{int}   $, visualized in Figure \ref{fig:normal}, is defined as $\jump{ a } =    a^{+} - a^{-} $
