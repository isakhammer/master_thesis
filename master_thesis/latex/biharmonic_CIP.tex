...
We define the two relevant CIP formulations for the \eqref{eq:bi_problem} as follows. Recall that we defined $V_{h} = \left\{ \mathcal{P} _{c}^{k}( \mathcal{T} _{h}) \text{ or } \mathcal{Q} _{c}^{k}( \mathcal{T} _{h})    \right\}$ in \eqref{def:Vh_background}.
