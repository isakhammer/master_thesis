...
orthogonality, $
P_{F} \nabla u  \cdot Q_{F}  \nabla u = 0$, holds. The normal projection matrix is defined as $Q_{F}(x) = n \otimes n $ and the tangential decomposition follows from $ P_{F} = I - Q_{F} = I - n \otimes n  =  \sum_{i=1}^{d-1} t_{i} \otimes t_i$, which
is a orthonormal basis $t_{i}$, $i = 1, \ldots, d-1$ for the space orthogonal to the outer normal vector $n$ on a facet $F$. Let $ a_{1}, a_{2}, a_{3} \in R^{d}$ be any vectors, then it is well known that the following identity holds $ ( a_{1} \otimes a_{2}  ) a_{3} = ( a_{2}\cdot  a_{3}) a_{1} $. Hence, we have \[
    \begin{split}
   P_{F} \nabla u & =  (n \cdot \nabla u)n \\
   Q_{F} \nabla u & =   \nabla u  - (n \cdot \nabla u)n =  \sum_{ i =1 }^{d-1} ( t_{i}\cdot  \nabla u ) t_{i}
    \end{split}
\]
...
& =  ( \partial _{nn} u ), \partial _{n} v  )_{\partial T}+\sum_{i=1}^{d-1} ( \partial _{n t_{i}}u , \partial _{t_{i}} v  )_{\partial T}
    \end{split}
\]
Here we used that $n^{T}\cdot n = 1$ and $t_{i}^{T} \cdot t_{i} = 1 \ \forall i$, which holds since every facet in $\partial T$ is flat.
...
\partial_n (\partial_{t_{i}} u)  & = \nabla (\partial_n u) \cdot t_{i} = (D^2 u \cdot t_i) \cdot n = n^{T} D^2 u \ t_{i} = \partial _{nt_{i}} u.
    \end{align*}
