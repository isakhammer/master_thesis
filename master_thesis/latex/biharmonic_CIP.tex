...
\begin{lemma}
 Assume $u \in H^{4}( \Omega ) $ and $v \in  V_{h}$. Then we have the following identity.
\[
    \begin{split}
...
We add consistent symmetry terms $( \mean{ \Delta  v } ,\jump{ \partial _{n} u }    )_{\mathcal{F}^{int}_{h} } $ and $( \Delta  v  , \partial _{n} u     )_{\Gamma  } -( \Delta  v  , g_{1}  )_{\Gamma  }  $, which effectely is adding zero
