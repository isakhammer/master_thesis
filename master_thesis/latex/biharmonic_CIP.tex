...
Under the assumption that $g_{1} = 0$ on $\Gamma$, and given that the tangent vector is orthogonal to $n$, we can assert that $ \partial {t{i} n} u = \partial {t{i}} ( \partial _{n} u ) = 0 $ for any index $i = 1,\ldots, d$. This implies that the
