...
& =  ( \partial _{nn} u ), \partial _{n} v  )_{\partial T}+\sum_{i=1}^{d-1} ( \partial _{n t_{i}}u , \partial _{t_{i}} v  )_{\partial T}
    \end{split}
\]
Here we used that $n^{T}\cdot n = 1$ and $t_{i}^{T} \cdot t_{i} = 1 \ \forall i$.
...
\partial_n (\partial_{t_{i}} u)  & = \nabla (\partial_n u) \cdot t_{i} = (D^2 u \cdot t_i) \cdot n = n^{T} D^2 u \ t_{i} = \partial _{t_{i}n} u.
    \end{align*}
...
= &\sum_{T\in \mathcal{T} _{h}}^{} ( D^2u,D^2v ) _{T }  + \sum_{F \in \mathcal{F}_{h}^{ext} }^{}  (\partial _{n} \Delta  u,v)_{F} - (\partial _{nn} u, \partial _{n}v )_{F}  - \sum_{i=1}^{d-1} ( \underbrace{\partial _{ t_{i}n} u }_{  \partial_{ t_{i} } g_{1},  }, \partial _{t_{i}} v
)_{F}     \\
...
Remark that we can simplify the exterior boundary integral term by first adding $t_{i}t_{i}^{T}=1$ and then apply \eqref{eq:projection},
\[
    \begin{split}
\sum_{{i} =1}^{d-1} (\partial _{t_{i}} g_{1} , \partial _{t_{i}} v )_{F } & =\sum_{{i} =1}^{d-1} ( t_{i}^{T} \nabla  g_{1} \ t_{i} , t_{i}^{T} \nabla  v \ t_{i} )_{F } \\
                                                                          & = ( (I - n  \otimes n ) \nabla _{} g_{1}  , (I - n  \otimes n ) \nabla  v  )_{F },  \quad F \in \mathcal{F} ^{ext}_{h}
    \end{split}
\]
