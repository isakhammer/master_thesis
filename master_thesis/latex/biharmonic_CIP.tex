...
We will now assemble the Hessian CIP formulation. Assume that $g_{1} = 0$ holds. We want to find a $u \in H^{3}( \Omega ) $ such that \begin{equation}
    a^{H}( u, v_{h})  = l_{h}( v_{h} )  \quad \forall v_{h} \in  V_{h}
\end{equation}
where the following bilinear and linear forms are
\begin{equation}
...
Remark that we have the consistent terms $\left( \mean{  \partial _{n n} u }, \jump{ \partial _{n }v_{h}} \right)_{\mathcal{F}_{h}^{int}} $ and $ \left(   \partial _{n n} u ,  \partial _{n }v \right)_{\Gamma }$, which naturally appears in the
derivation. However, we also added two symmetry terms, $ \frac{\gamma }{h}  \left( \jump{ \partial _{n} u}, \jump{ \partial _{n} v   }   \right)_{\mathcal{F}_{h}^{int}}$ and $\left( \mean{  \partial _{n n} u }, \jump{ \partial _{n }v_{h}}
\right)_{\mathcal{F}_{h}^{int}} $ and $ \left(   \partial _{n n} u ,  \partial _{n }v \right)_{\Gamma }$, and the so-called penalty terms $ \frac{\gamma }{h}  \left(  \partial _{n} u,  \partial _{n} v      \right)_{\Gamma }$.
     What we will see later that these term is essential for making the problem well-posed, hence, the name interior penalty method or symmetric interior penalty method. For more information of CIP nonconform error analysis, see \cite[Chapter 1.3]{pietro2012}.
\end{remark}
