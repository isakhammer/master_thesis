...
Remark that we can simplify the exterior boundary integral term for each $F \in \mathcal{F} ^{ext}_{h}$ by first adding $t_{i}t_{i}^{T}=1$ and then apply \eqref{eq:projection},
\[
    \begin{split}
...
\begin{split}
\left( \Delta  ^{2} u, v \right) _{\Omega }  =&   \left( D^2u, D^2v \right)_{\mathcal{T} _{h}} +  \left(g_{2}, v  \right) _{\mathcal{F}^{ext}_{h} }  -  ( \mean{ \partial _{nn} u }   , \jump{ \partial_{n} v } )_{\mathcal{F}_{h} }\\ & -( (I - n  \otimes n ) \nabla _{} g_{1}  , (I - n  \otimes n ) \nabla  v  )_{ \mathcal{F}^{ext} _{h}  }
\end{split}
\end{equation}
