...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \foreach \x/\y in {0/1.0, 1/1.7, 2/2.5, 3/1.2} {
      \fill (\x,\y) circle (2pt);
    }

    % Draw linear interpolation between points
    \draw[blue, thick] (0,1.0) -- (1,1.7) -- (2,2.5) -- (3,1.2);
  \end{tikzpicture}
  \caption{ Illustration of $C^{0}$ continuous elements using $P^{1}$.}
  \label{fig:xyplot_quadrant_nodes_linear_interpolation}
\end{figure}
