...
\begin{figure}[h!]
    ...
    \end{tikzpicture}
    \end{minipage}
    \caption{ Illustration of global $C^{0}$ continuous elements (left) and global $C^{1}$ elements (right) in 1 dimension. Here, $u_{h}$ is a discrete solution in $\mathcal{P}^2$, where the jump between the elements is denoted as $ \jump{ u_{h} } = u_{+} - u_{-}$. }
\label{fig:global_C0}
\end{figure}
