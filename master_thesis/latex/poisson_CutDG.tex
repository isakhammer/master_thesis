...
\begin{corollary}[Unfitted inverse estimates]
    ...
    Assume that $v \in V^{h} $ and let $\mathcal{T}_{h} $ be a active mesh, then does the following estimates hold.
    \[
\begin{split}
    \| \partial _{n} v \|_{ \mathcal{F}_h \cap \Omega  }^{  } & \le \| \partial _{n}  v \|_{\mathcal{F}_{h}   }^{  } \le  C_{I} h ^{-\frac{1}{2}} \| \nabla v \|_{ \mathcal{T}_{h}  }^{  } \\
    \| \partial _{n} v \|_{ \mathcal{T}_{h} \cap \Gamma   }^{  } &  \le  C h ^{-\frac{1}{2}} \| \nabla v \|_{ \mathcal{T} _{h} }^{  } \\
\end{split}
    \]
    Or equivalently,
    \[
\begin{split}
    h^{\frac{1}{2}}\| \partial _{n} v \|_{ \mathcal{F}_h \cap \Omega  }^{  } & \le \| \partial _{n}  v \|_{\mathcal{F}_{h}   }^{  } \le  C_{I}  \| \nabla v \|_{ \mathcal{T}_{h}  }^{  } \\
    h^{\frac{1}{2}}\| \partial _{n} v \|_{ \mathcal{T}_{h} \cap \Gamma   }^{  } &  \le  C  \| \nabla v \|_{ \mathcal{T} _{h} }^{  } \\
\end{split}
    \]
\end{corollary}

Remark that the $C_{\Gamma } $ is dependent on the curvature of $\Gamma $. \todo[inline]{ TODO: Look into\cite{hansbo2003finite} and actually
find the proof.}
...
\left\lvert g_{h}( v,u)  \right\rvert \le \left\lvert v \right\rvert _{g_{h}} \left\lvert u \right\rvert _{g_{h}} \le \| v \|_{ A_{h} }^{  } \| u \|_{ A_{h} }^{  }
    ...
    Looking at each terms individually can we observe that each terms is bounded, as follows,
    \begin{itemize}
        \item By applying the assumption \ref{as:est_EP1} and Corollary \ref{cor:g_h_inverste_results} we get \[
\begin{split}
...
\end{split}
 ...
 &  \le ( \| \nabla v \|_{ \Omega  }^{  } + \abs{ v } _{g_{h}} ) \|h^{-\frac{1}{2}} u \|_{ \Gamma    }^{  } \\
                 & \le \| v \|_{A_{h}   }^{  } \| u \|_{ A_{h}    }^{  }
                \end{split}
            \]
            \todo[inline]{ Wait, here we used the $ \| \cdot  \|_{ a_{h},* }^{  } $ instead of $\| \cdot  \|_{ A_{h} }^{  } $ ??     }
            Here we used the Corollary \ref{cor:unfitted_inverse_estimates} in the first step and then applied the Assumption \ref{as:est_EP1}.
        ...
        \item For the jump terms \[
                \begin{split}
        \left\lvert( \mean{ \partial _{n} v }  , \jump{ u }  ) _{\mathcal{F} _{h} \cap \Omega }  \right\rvert \le \ldots
                \end{split}
        \]
    \item And for the DG penalty term
        \[
         \left\lvert \beta ( h^{-1} \jump{ v }  ,
    \jump{ u }  )_{\mathcal{F}_{h}\cap \Omega  } \right\rvert \le  \ldots
        \]
    ...
    % \[
    % \begin{split}
    %      \| v \|_{ \Gamma   }^{  } \| \partial _{n}u \|_{ \Gamma  }^{  } & \le  \ldots. \\
    %      \beta \|h^{-1} v  \|_{\Gamma   }^{  }  \| u  \|_{\Gamma   }^{  } &\le \ldots \\
 % \| \mean{ \partial _{n}v }   \|_{\mathcal{F} _{h}\cap \Omega   }^{  } \| \jump{ u }   \|_{ \mathcal{F} _{h} \cap \Omega  }^{  }&  \le \ldots\\
 % \| \jump{ v }   \|_{ \mathcal{F}_{h} \cap \Omega   }^{  }  \| \mean{ \partial _{n} u }    \|_{ \mathcal{F} _{h} \cap \Omega  }^{  } & \le \ldots \\
% \beta \|h^{-1} \jump{ v }    \|_{\mathcal{F} _{h} \cap \Omega  }^{  }      \| \jump{ u } \|_{\mathcal{F} _{h} \cap \Omega   }^{  }& \le \ldots \\
    % \end{split}
    % \]
