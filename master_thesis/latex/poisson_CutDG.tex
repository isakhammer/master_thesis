...
\todo{ Ask Andre about the difference between $\Omega _{e}$, $\Omega ^{e}$  and $\Omega _{e,h}$. }
...
\| e_{h} \|_{ A_{h} }^{ 2 } & \lesssim a_{h}( \pi _{h}^{e} u - u_{h}, e_{h}) + g_{h}( \pi _{h}^{e}u - u_{h}, e_{h}) \\
 ...
 & \lesssim h^{r-1} \| u \|_{ r, \Omega  }^{  } \| e_{h} \|_{ A_{h} }^{  }.
    \end{split}
\]
On the last two lines we first applied Lemma \ref{lemma:Ah_bounded} and Assumption \ref{as:weak_consistency_estimate}, that is, \[
    \begin{split}
...
\]
\todo[inline]{ Explain this definition.  }
