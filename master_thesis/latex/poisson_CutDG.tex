...
\end{equation}
\todo[inline]{ What is an $\| \cdot  \|_{ \mathbb{R} ^{N} }^{  } $  norm?? }
...
\begin{lemma}
    Let $T_{1}, T_{2} \in  \mathcal{T} _{h} $. Then for $v_{h} \in  V_{h}$  does this hold \[
    \| v \|_{ T_{2} }^{  }  + \sum_{j=0}^{k}  h^{2j +1} ( \jump{ \partial _{n}^{j} v}, \jump{ \partial _{n}^{j} }    )
    \]
\end{lemma}
