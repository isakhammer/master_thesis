...
\todo[inline]{ Ask Andre about the difference between $\Omega _{e}$, $\Omega ^{e}$  and $\Omega _{e,h}$. }
...
\todo[inline]{ What is the definition of an $\| \cdot  \|_{ \mathbb{R} ^{N} }^{  } $  norm?  Is it what we know as $\| \cdot  \|_{ 2 }^{  } $?    }
 ...
 Hence, there is a equivalent the discrete $l^{2} $-norm to $L^{2}$-norm. We now want to connect the discrete norm to the energy norm, $A_{h}$. This can be fulfilled using an extension on the ghost penalty, that is,
