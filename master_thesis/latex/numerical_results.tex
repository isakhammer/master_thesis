...
\]
Because of the connection of $l^{2}$ norms of a matrix and its eigenvalues is $k_{2}( A) $ preferred in most cases numerical analysis. However, from linear algebra is a well known result that $ \| A \|_{ \infty }^{  }  \le \| A \|_{2  }^{  } \le
\sqrt{N}  \| A \|_{\infty  }^{  }  $ \footnote{
...
}, hence, we can bound the condition number applying the $l^{\infty}$ norm,
\[
...
\begin{itemize}
    \item \textbf{Convergence on Circe domain}
        ...
        \[
            u(x,y) = (x^2+ y^2 -1) \sin\left( \pi x \right)\cos\left(\pi y\right)
        \]
        The result can be shown on the in Figure \ref{fig:conv_hes_lap} and Table \ref{tab:conv_hes_lap}. Here we observe that for we get optimal convergence for $\| \cdot  \|_{a_{h},*  }^{  } $ and the $ \| \cdot  \|_{L^{2}( \Omega )   }^{  } $
        norm.
