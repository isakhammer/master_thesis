...
Recall that $V_{h} $ is spanned by the polynomial basis $\left\{ \phi _{1}, \ldots, \phi _{N}  \right\} $ s.t. $dim(V_{h}) = N$, where $N$ is the so-called number of degrees of freedom (ndofs). From basic theory
