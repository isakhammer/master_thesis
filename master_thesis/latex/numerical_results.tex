...
examples where it holds: circular domain and a flower domain. We also demonstrate the effect ghost penalty of stabilization by translating the domain to trigger badly cut cells. Finally, we provide numerical validation of the expected convergence for
the Cahn-Hilliard problem.

We propose the following penalty parameters $\gamma , \gamma _{g1}, \gamma _{g2} = 20,10, 0.1$.It is worth noting from the results provided in \cite{brenner2012, brenner2012quadratic_kirk,
...
\begin{itemize}
    \item EOC Test on circle and its configuration
        \begin{enumerate}[label=\arabic*)]
            \item Background mesh $L = 3.11$ and circle $R=1$, flower $r_0, r_1 = 0.3L, 0.1L$
        \end{enumerate}
    \item Translation
        \begin{enumerate}[label=\arabic*)]
...
\[
u_{\text{ex}}(x,y) = (x^2- y^2 -1) \sin\left(\frac{2\pi m}{l}x\right)\cos\left(\frac{2\pi r}{l}y\right)
\]
