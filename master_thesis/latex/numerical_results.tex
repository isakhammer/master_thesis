...
corresponding ghost penalty \eqref{eq:ghost_penalty}. It is worth noting from the results provided in \cite{brenner2012, brenner2012quadratic_kirk,
...
The big-oh $O( h^{r})$ is defined to be to be a upper bound such that it exists an $C>0$ s.t.  $\| e \|_{  }^{  } \le M h^{r} \text{ for all } h$, where $r$ is the order of convergence. Since we only will implement $k=2$, we validate that it exists an upper bound $ O( h)  $ for  $\| e \|_{L^{2}( \Omega )   }^{  }$  and $O( h^2)  $ for $\| e \|_{ a_{h},* }^{  } $.
...
\begin{itemize}
    \item \textbf{Convergence on circe domain}
        ...
        Remark that we get a slight drop for high refinements due to high condition number, which is expected since it has been shown that the condition number $\kappa_{2} ( A) $  increases with the order of $O( h^{-4}) $\cite{li07}.
