...
We define big-oh $O( h^{r})$ to be a upper bound s.t. it exists an $C>0$ s.t.  $\| e \|_{  }^{  } \le M h^{r} \text{ for all } h$, where $r$ is the order of convergence.
...
\begin{itemize}
    \item \textbf{Convergence on Circe domain}
        ...
        \]
        The result can be shown on the in Figure \ref{fig:conv_hes_lap} and Table \ref{tab:conv_hes_lap}. Here we observe that for we clearly get optimal convergence, that is, for $\| \cdot  \|_{a_{h},*  }^{  } $ we get a consistent $EOC$ of $1$
        hence,  and the $ \| \cdot  \|_{L^{2}( \Omega )   }^{  } $,
        norm.
