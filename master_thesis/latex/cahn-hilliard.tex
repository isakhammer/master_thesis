...
From \cite{yushutin19} can we observe the energy functionals
\begin{equation*}
   E _{1}(c) = \| f(c) \|_{ \Gamma  }^{ 2 }
.\end{equation*}
where \[
f\left( c \right) = f_{0}\left( c \right) + \frac{1}{2}\varepsilon ^{2} \left\lvert \nabla _{\Gamma } c \right\rvert ^{2}
\]
and the conservation law  $\rho \frac{\partial c}{\partial  t}  + div_{\Gamma } \mathbf{j} = 0$, derived from the Ficks Law $\mathbf{j} = - M \nabla _{\Gamma } \mu $ for the chemical potential derived by the functional derivative $\mu = \frac{\delta  f}{\delta c} $.
