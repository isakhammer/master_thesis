...
and $t_{M} = T$. Here, each time step is an element of the function space, i.e. $U^{m} \in V_{h}$  with the initial condition defined as $U^{0} = u( t_{0},x )$.
For the Implicit explicit scheme (IMEX) sceheme we have the following discretization
\[
( \overline{\partial } _{t} U^{m}, v   )_{\Omega } + \varepsilon a^{m}_{h}( U^{m} , v) + \frac{1}{\varepsilon } c_{h} (  U^{m-1}, v)  =0, \quad \forall v \in V_{h}^{m}.
\]

Here we have define the forward difference operator with time step $\tau $
\[
\overline{\partial } _{t} U^{m} = \frac{U^{m} - U^{m-1}}{ \tau }
\]
Hence, we have a explicit scheme
\[
U^{m}  + \tau \varepsilon a_{h}( U^{m} , v)   =  U^{m-1} - \frac{\tau}{\varepsilon } c_{h} (  U^{m-1}, v).
\]
