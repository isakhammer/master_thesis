...
\newpage
\section{Demonstration of solving the Cahn Hilliard equation }%
\label{sec:cahn_hilliard_equation}

In this paper, we will demonstrate that the proposed cut finite element method can be used to solve the Cahn-Hilliard problem. Firstly, we will recall the strong form and illustrate how it can be recast into a weak form following the approach in
...
\partial _{n}    \Delta u       &= 0 \quad \text{ on } \Gamma  \\
    \end{split}
\end{equation}
Where we define $f( u) = F' ( u)  $ where $F( u) = ( 1 / 4 ) ( u^2 -1 ) ^{2} $. With the corresponding energy.
\begin{equation}
