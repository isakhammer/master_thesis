...
Recall definition $V_{h}$ in \eqref{eq:vh_energy} of polynomial degree $k$ and starting from Lemma \ref{lemma:clements} and the corresponding estimates \eqref{eq:clement_estimates}, we combine the Clément interpolator with the extension operator to construct $C_{h}^{e}: H^{m}( \mathbb{R} ^{d}) \to V_{h}$ such that $C ^{e} _{h} v := C _{h} v^{e} $.
\begin{align}
    \label{eq:bi_projection_estimates_1}
    \| v - C _{h}^{e} v \|_{  l, \mathcal{T} _{h} }^{  } & \lesssim h^{r-l} \| v \|_{ r, \Omega }^{  }, \quad 0\le l\le r, \\
    \label{eq:bi_projection_estimates_2}
    \| v - C ^{e}_{h}v \|_{ l,\mathcal{F} _{h} }^{  } & \lesssim h^{r-l-\frac{1}{2}} \| v \|_{ r, \Omega  }^{  }, \quad 0  \le  l \le   r-\frac{1}{2}, \\
    \label{eq:bi_projection_estimates_3}
\| v - C ^{e}_{h}v \|_{ l, \Gamma }^{  } & \lesssim h^{r-l-\frac{1}{2}}   \| v \|_{ r,  \Omega  }^{  }, \quad 0  \le  l \le  r-\frac{1}{2}.
\end{align}
