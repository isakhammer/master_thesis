...
& \lesssim \sum_{T \in \mathcal{T}_{h} }^{}  h^{ 2(s - \frac{5}{4} )  } \| u \|_{ s, \mathcal{T}_{h}    }^{  2} \\
    ...
    And for the boundary term we do a similar procedure, but instead use \eqref{eq:bi_projection_estimates_3} and the first order inverse estimate \eqref{eq:bi_n_cut_inverse_1}. \[
        \begin{split}
            \gamma \| h^{-\frac{1}{2}}  \partial _{n} ( u - C_{h}^{e}u ) \|_{ \Gamma    }^{  2} & \lesssim h^{-2} \|   \nabla  ( u - C_{h}^{e}u )    \|_{ \mathcal{T}_{h}   }^{2  }  \lesssim  h^{-2} \|    u - C_{h}^{e}u     \|_{1,2, \Gamma  }^{2  } \\
...
\item
 ...
 \| h^{\frac{1}{2}} \mean{ \partial _{nn} (u - C_{h}^{e}u) }   \|_{\mathcal{F} _{h}^{} \cap \Omega   }^{  2} &  \lesssim h^{} \|   \partial _{nn} (u - C_{h}^{e}u)    \|_{\partial \mathcal{T} _{h} \cap \Omega    }^{  2}  \lesssim h^{} h^{-1}  \|   D^2 (u - C_{h}^{e}u)    \|_{ \mathcal{T} _{h}   }^{  2} \\
            ...
            \item
                ...
                \[
              \| h^{\frac{1}{2}} \partial _{nn}(u - C_{h}^{e}u)     \|_{ \Gamma }^{  2} \lesssim   \|  D^2(u - C_{h}^{e}u)     \|_{ \mathcal{T}_{h}   }^{  2} \lesssim \|  u - C_{h}^{e}u \|_{ 2, \mathcal{T}_{h} }^{2  } \lesssim h^{2(s-2)} \sum_{T \in
