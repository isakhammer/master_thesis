...
\subfloat[]{
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        \fill (ahi) circle (2pt);
        \node[anchor=north west] at (ahi) {$\hat{a}_i$};
...
\subfloat[]{
    ...
    \node[anchor=north west] at (ahat_i) {$\hat{a}_i$};
...
\subfloat[]{
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        \node[anchor=south, yshift=0.2cm] at (ah_i) {$\hat{a}_i$};

        % Reference hexagon
        \foreach \angle in {0, 60, ..., 300} {
            \coordinate (A) at (\angle:2);
...
\caption{Illustration of the mapping from the reference macroelement $\widehat{A}_{j( i) }$  to the domain $A_{i}$,  $G_{A_{i}}: \widehat{A}_{j( i) } \to A_{i}$. Here we have defined $\hat{a}_{j(i)} \in \widehat{A}_{j(i)}$ s.t. $G( \hat{a}_{j( i) })
= a_i$. }
\end{figure}
