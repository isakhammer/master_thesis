...
However, the discrete solution $C^{0}$ makes it difficult to apply the standard Lagrange interpolation operator. Hence, this motivates us to introduce a method to interpolate a non-smooth function, the so-called Cléments interpolation operator.
...
\begin{figure}[ht]
  ...
  \centering
    ...
    \caption{Illustration of patch $\omega ( F) $ on left side and $\omega(T)$ on the rights side.   }
