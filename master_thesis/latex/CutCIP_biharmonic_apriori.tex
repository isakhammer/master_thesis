...
\eqref{eq:bi_projection_estimates_2} was never used in the proof of Lemma \ref{lemma:astar_estimate} since we used inverse estimates and ended up with \eqref{eq:bi_projection_estimates_1} instead on all of them.}
...
& \lesssim   h^{ 2(s - 2)  } \| u \|_{ s, \mathcal{T}_{h}   }^{  2} \\
    ...
    And for the boundary term we do a similar procedure, but instead use the first order inverse estimate \eqref{eq:bi_n_cut_inverse_1}. \[
        \begin{split}
            ...
            & \lesssim h^{-2} h^{2(s-1)}   \| u \|_{s, \mathcal{T}_{h}   }^{  2} \\
            & \lesssim  h^{2(s- 2)}   \| u \|_{s, \mathcal{T}_{h}   }^{  2}
        \end{split}
    \]
