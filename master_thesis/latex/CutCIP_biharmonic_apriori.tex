...
\begin{proof}
    ...
    \[
        ...
        \end{split}
    \]
    Here we simply used \eqref{eq:bi_projection_estimates_1} and argue that $ \sum_{T}^{} \| v \|_{s,\omega ( T)   }^{  } \lesssim \| v \|_{s, \mathcal{T}_{h}   }^{  } $.

\item
        Recall $\| \jump{ \partial _{n} u }   \|_{ \mathcal{F} _{h} }^{  } \le \| \partial _{n^{+}} u^{+}   \|_{ \mathcal{F} _{h} }^{  } +   \|\partial _{n^{-}} u^{-}   \|_{ \mathcal{F} _{h} }^{  } \lesssim  \|\partial _{n} u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $
        and the inverse estimate $\| \partial _{n} u \|_{ F  }^{2} \lesssim h^{-1} \| \nabla u \|_{ T }^{2  }  $.
     Hence, by \eqref{eq:bi_projection_estimates_2} we have \[
        \begin{split}
...
&= \sum_{T \in \mathcal{T}_{h} }^{} h^{ 2(s -\frac{3}{2})  } \| u \|_{ s, \omega (  F)  }^{  2} \\
        \end{split}
    \]
\item
