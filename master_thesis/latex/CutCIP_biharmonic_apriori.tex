...
A key idea is to distinguish between the physical space $\Omega $ and the polyhedra consisting of the active mesh $\Omega ^{e}_{h} = \mathcal{T}_{h}$. Assume that $\Omega $ has a boundary $\Gamma $ in $C^{1}$ that does exists an bounded extension operator, \[
...
\begin{remark}
    Be aware that this theorem requires that we have a sufficiently smooth domain, hence, emphasizing the assumption of a sufficiently smooth boundary. Also keep in mind that the original results is the theorem a mapping from $ ( \cdot ) ^{e}: W^{m,q}(
    \Omega )  \to W^{m,q} ( \mathbb{R} ^{d})$, but we restricted ourself to the active set $\Omega ^{e}_{h} = \mathcal{T} _{h}$ .
    \todo[inline]{ TODO: Not sure if this argument holds. At least from my point of view is this choice naive without proof. }
\end{remark}
