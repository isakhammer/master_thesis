...
\| v^{e} \|_{ m,q,\mathbb{R} ^{d}  }^{  } & \lesssim \| v \|_{ m,q, \Omega  }^{  }.
    \end{split}
\]
To simplify, we use the notation $ v := v^{e}   $ for $v \in \mathbb{R} ^{d} \setminus \Omega $.
...
%     \todo[inline]{ TODO: Not sure if this argument holds. At least from my point of view is this generalization not trivial.}
% \end{remark}
...
Recall the Lemma \ref{lemma:clements}, we construct the Cléments interpolator for the extension operator $C_{h}^{e}: H^{m}( \mathbb{R} ^{d}) \to V_{h}$ such that  $C ^{e} _{h} v := C _{h} v^{e} $.
We can immediately observe that the interpolation satisfies the global error estimates, that is,
\begin{align*}
    \| v - C _{h}^{e} v \|_{  m, \mathcal{T} _{h} }^{  } & \lesssim h^{l-m}\sum_{T \in \mathcal{T}_h} \| v \|_{ l, \omega(T) }^{  }, \quad m\le l\le k+1 \\
    \| v - C ^{e}_{h}v \|_{ m,\mathcal{F} _{h} }^{  } & \lesssim h^{l-m-\frac{1}{2}}\sum_{T \in \mathcal{T}_h} \| v \|_{ l, \omega(F)  }^{  }, \quad m+\frac{1}{2}  \le  l \le   k+1 \\
\| v - C ^{e}_{h}v \|_{ m, \Gamma }^{  } & \lesssim h^{l-m-\frac{1}{2}} \sum_{T \in \mathcal{T}_h}  \| v \|_{ l,  \omega(T)  }^{  }, \quad m+\frac{1}{2}  \le  l \le  k+1
\end{align*}
