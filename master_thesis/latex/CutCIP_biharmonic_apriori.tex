...
\begin{proof}
    ...
    \[
        \begin{split}
            ...
            \| D^2 ( u - C_{h}^{e}u ) \|_{\mathcal{T} _{h} \cap \Omega   }^{ 2 } &  \lesssim  \|  u - C_{h}^{e}u  \|_{2,\mathcal{T} _{h}   }^{ 2 }   =  \|  u - C_{h}^{e}u  \|_{2,2,\mathcal{T} _{h}   }^{ 2 } \\
                                                                                 & \lesssim \sum_{T \in \mathcal{T} _{h}}^{} h^{2(s-2)} \| u \|_{ s, \omega ( T)  }^{ 2 }\lesssim \sum_{T \in \mathcal{T} _{h}}^{} h^{2(s-2)} \| u \|_{ s, \mathcal{T}_{h}}^{ 2 }.
        \end{split}
    \]
    Here we simply used \eqref{eq:bi_projection_estimates_1} and argue that $ \sum_{T}^{} \| v \|_{s,\omega ( T)   }^{  } \le C  \| v \|_{s, \mathcal{T}_{h}   }^{  } $ where $C$ is some constant decided by the maximum number of elements in a patch
    $\omega( T)  $ for all $T \in \mathcal{T} _{h}$. This also holds for the inequality $ \sum_{T}^{} \| v \|_{s,\omega ( F)   }^{  } \le C  \| v \|_{s, \mathcal{T}_{h}   }^{  } $.
    ...
    \|\partial _{n^{-}} u^{-}   \|_{ \mathcal{F} _{h} }^{  } \lesssim  \|\partial _{n} u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $.
        Hence, by using the inequality $\| \partial _{n} u \|_{ \partial  \mathcal{T}_{h}   }^{2} \lesssim h^{-1} \| \nabla u \|_{ \mathcal{T}
        _{h} }^{2  }  $ and  by \eqref{eq:bi_projection_estimates_2} we have \[
        \begin{split}
...
&=  h^{-2} \|    u - C_{h}^{e}u     \|_{ 1,2,\mathcal{T} _{h} }^{2  } \lesssim \sum_{T \in \mathcal{T}_{h} }^{} h^{ 2(s - 1 - \frac{1}{2})  } \| u \|_{ s, \omega (  F)  }^{  2} \\
& \lesssim  h^{ 2(s -2)  } \| u \|_{ s, \mathcal{T}_{h}    }^{  2} \\
    ...
    And for the boundary term we simply apply \eqref{eq:bi_projection_estimates_3}, \[
        \begin{split}
            ...
            & \lesssim h^{2(s- 1- \frac{1}{2}) -2} \sum_{T \in \mathcal{T} _{h}}^{}  \| u \|_{s,T  }^{  2}
        \end{split}
    \]
