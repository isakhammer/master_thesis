...
\end{align}
\todo[inline]{ Maybe hard to argue \eqref{eq:bi_projection_estimates_3} to hold on $\Gamma $.}
...
\begin{proof}
    ...
    The strategy is to bound each term individually by applying the estimates \eqref{eq:bi_projection_estimates_1}, \eqref{eq:bi_projection_estimates_2} and \eqref{eq:bi_projection_estimates_3}.

    \begin{enumerate}[label=\arabic*)]
        \item     Starting with the first two terms we get
    \[
        \begin{split}
            \| |\alpha |^{\frac{1}{2}} ( u - C_{h}^{e}u) \|_{ \mathcal{T} _{h} \cap \Omega  }^{ 2}    & \lesssim     \|  ( u - C_{h}^{e}u) \|_{0,\mathcal{T} _{h}  }^{ 2}
             =\|  ( u - C_{h}^{e}u) \|_{0,2,\mathcal{T} _{h}  }^{ 2} \\
             &   \lesssim  h^{2s} \sum_{T \in \mathcal{T}
            _{h}}^{}   \| u \|_{s,w( T)  }^{  2} \lesssim  h^{2(s-2)} \sum_{T \in \mathcal{T}
            _{h}}^{}   \| u \|_{s,\mathcal{T}_{h} }^{  2} \\
            \| D^2 ( u - C_{h}^{e}u ) \|_{\mathcal{T} _{h} \cap \Omega   }^{ 2 } &  \lesssim  \|  u - C_{h}^{e}u  \|_{2,\mathcal{T} _{h}   }^{ 2 }   =  \|  u - C_{h}^{e}u  \|_{2,2,\mathcal{T} _{h}   }^{ 2 } \\
                                                                                 & \lesssim \sum_{T \in \mathcal{T} _{h}}^{} h^{2(s-2)} \| u \|_{ s, \mathcal{T}_{h}}^{ 2 }.
        \end{split}
    \]
    Here we simply used \eqref{eq:bi_projection_estimates_1}.

    Recall the inverse estimate $\| \partial _{n} u \|_{ F  }^{2} \lesssim h^{-1} \| \nabla u \|_{ T }^{2  }  $ and that $\| \jump{ \partial _{n} u }   \|_{ \mathcal{F} _{h} }^{  } \le \| \partial _{n^{+}} u^{+}   \|_{ \mathcal{F} _{h} }^{  } +
    \|\partial _{n^{-}} u^{-}   \|_{ \mathcal{F} _{h} }^{  } \lesssim  \|\partial _{n} u \|_{ \partial\mathcal{T }_{h}  }^{2  }  $.
        Thus, by the inequality $\| \partial _{n} u \|_{ \partial  \mathcal{T}_{h}   }^{2} \lesssim h^{-1} \| \nabla u \|_{ \mathcal{T}
        _{h} }^{2  }  $ and  by \eqref{eq:bi_projection_estimates_2} we see, \[
        \begin{split}
...
&=  h^{-2} \|    u - C_{h}^{e}u     \|_{ 1,2,\mathcal{T} _{h} }^{2  } \lesssim \sum_{T \in \mathcal{T}_{h} }^{} h^{ -2 } h^{ 2(s - 1 - \frac{1}{2})  } \| u \|_{ s, \omega (  F)  }^{  2} \\
& \lesssim \sum_{T \in \mathcal{T}_{h} }^{}  h^{ 2(s - \frac{5}{4} )  } \| u \|_{ s, \mathcal{T}_{h}    }^{  2} \\
        \end{split}
        .
    \]
    \red{ Something wrong at $\frac{5}{4}$ since it is not $2$. This comes from an $h^{-\frac{1}{2}}$ too much.}
    And for the boundary term we do a similar procedure, but instead use \eqref{eq:bi_projection_estimates_3}. \[
        \begin{split}
            \gamma \| h^{-\frac{1}{2}}  \partial _{n} ( u - C_{h}^{e}u ) \|_{ \Gamma    }^{  2} & \lesssim h^{-2} \|   \nabla  ( u - C_{h}^{e}u )    \|_{ \Gamma  }^{2  }  \lesssim  h^{-2} \|    u - C_{h}^{e}u     \|_{1,2, \Gamma  }^{2  } \\
            & \lesssim h^{-2} h^{2(s- 1- \frac{1}{2})}   \| u \|_{s, \mathcal{T}_{h}   }^{  2} \\
            & \lesssim  h^{2(s- \frac{5}{4})}   \| u \|_{s, \mathcal{T}_{h}   }^{  2}
        \end{split}
    \]
    \red{Same $\frac{5}{4} \text{ or }h^{-\frac{1}{2}}$ issue as above}.
\item
            ...
            inequality \eqref{eq:bi_cut_inverse_1}. It is clear
            by using \eqref{eq:bi_projection_estimates_1} that this holds.
            \[
                \begin{split}
 \| h^{\frac{1}{2}} \mean{ \partial _{nn} (u - C_{h}^{e}u) }   \|_{\mathcal{F} _{h}^{} \cap \Omega   }^{  2} &  \lesssim h^{} \|   \partial _{nn} (u - C_{h}^{e}u)    \|_{\partial \mathcal{T} _{h}   }^{  2}  \lesssim h^{} h^{-1}  \|   D^2 (u - C_{h}^{e}u)    \|_{ \mathcal{T} _{h}   }^{  2} \\
                                                                                                                &  = \|   u - C_{h}^{e}u    \|_{ 2, \mathcal{T} _{h}   }^{  2}  \lesssim h^{2(s - 2)} \sum_{T \in \mathcal{T}_h}^{} \| u \|_{s, \mathcal{T} _{h} }^{  }
                \end{split}
            \]
            \item
                Similarly we can easily see by using \eqref{eq:bi_projection_estimates_1} and the second order boundary inverse inequality  \eqref{eq:bi_cut_inverse_2} that this must hold,

                \[
              \| h^{\frac{1}{2}} \partial _{nn}(u - C_{h}^{e}u)     \|_{ \Gamma }^{  2} \lesssim   \|  D^2(u - C_{h}^{e}u)     \|_{ \mathcal{T}_{h}   }^{  2} \lesssim \|  u - C_{h}^{e}u \|_{ 2, \Gamma}^{2  } \lesssim h^{2(s-r -\frac{1}{2})} \sum_{T \in
              \mathcal{T}_h }^{} \| u \|_{ s, \omega ( T)  }^{2  }
            \]
    \end{enumerate}

    Thus, all elements is bounded and the proof is complete.
\end{proof}

\begin{lemma}[Weak galerkin orthogonality]
Let $u \in H^{s}( \Omega )  $, $ s\ge 3 $  be the exact solution and $u_{h} \in V_{h}$ is a discrete solution. Then is \[
...
\begin{theorem}
    ...
    Let $u \in H^{s}( \Omega ) $ , $s\ge 3$ a solution and let $u \in V_{h}$ of order $k\ge 2$ be the solution to \eqref{eq:Bi_a_h}. Then for $r = \min_{}\{s, k+1\} $ the error $e = u - u_{h}$ satisfies
    \begin{align}
