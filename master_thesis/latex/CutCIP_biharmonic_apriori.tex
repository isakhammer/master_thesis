...
\begin{remark}
    ...
    \Omega )  \to W^{m,q} ( \mathbb{R} ^{d})$, but we restricted ourself to a slightly bigger extension $\Omega^{e} \supset   \Omega $.
    This is useful because we are now able to extend the function to the active set, while preserve Sobolev regularity.
...
Now construct the extended Sobolev space s.t. that $\Omega _{h}^{e} =\mathcal{T}_{h} \subset  \Omega^{e} $. We define an unfitted Cléments interpolator $C_{h}^{e}: H^{r}( \Omega ^{e}_{h}) \to V_{h}$
