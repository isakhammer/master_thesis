...
\begin{lemma}[Magic formula]
    ...
    $  \jump{ uv }    = \jump{ u }   \mean{ v }    + \jump{ u }  \mean{ v }     $.
\end{lemma}
\begin{proof}
    ...
    \[
        \begin{split}
    \jump{ uv }   & = u^{+}v^{+} - u^{-}v^{-} \\
     & = \frac{1}{2}(u^{+} + u^{-})( v^{+} - v^{-})  + \frac{1}{2}(v^{+} + v^{-})( u^{+} - u^{-}) \\
     & =  \jump{ u }   \mean{ v }    + \jump{ u }  \mean{ v }
        \end{split}
    \]
\end{proof}
...
\begin{lemma}[Local inverse estimate]
    ...
    \]
    Here does the constant $C_{I}>0$ depend on the dimension $d$.
\end{lemma}
...
\begin{definition}[Linear bounded function]
    \label{def:linear_function}
    Write definitions of a linear bounded operator
\end{definition}

\begin{definition}[Bilinear bounded function]
