...
$\sigma_{i}( p) = p( a_{i}) \  \forall p \in \mathcal{P}$,  then the triples $( T, \mathcal{P}, \Sigma  ) $ is called a Lagrangian finite element. The set of points $\left\{ a_{i} \right\}_{i \in N}  $ is called nodes and is associated with the
...
element as $\mathbb{P} ^{k}(T )$ and $\mathbb{Q} ^{k}( T)$, both of polynomial order $k$. In Figure \ref{fig:ill_nodes} is it illustrated for  $k=1,2,3$ in dimension of $d=2$ on how the node configuration evolve.
