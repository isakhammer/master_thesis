...
\caption{Facet $F \in \mathcal{F}_h^{int} $ shared by the triangles $T^{+}, T^{-} \in \mathcal{T}_{h} $ and the normal unit vector $n^{+}$ and $n^{-}$. If we pick $T=T^{+}$ then we define $n  \mid _{F} = n^{+}$.}
    \label{fig:normal}
\end{figure}
