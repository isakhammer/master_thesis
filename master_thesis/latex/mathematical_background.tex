...
In this section, we will revisit the established definitions and provide a brief overview of Sobolev spaces. Afterward, we will briefly review the finite element method and discuss the necessary tools required for calculating a priori estimates.
...
In this report will we generally specify any discrete space $V_{h}(\mathcal{T}_{h})$  as the $C^{0}$ continuous polynomial space $\mathcal{P}^{k}(\mathcal{T}_{h})$, of order $k$.
...
a_{j} \right) $, so that $u_{h} = \sum_{j=1}^{N} u_{j} \phi _{j}  $. Then the Problem \ref{def:approx_problem} is equivalent to
 \[
\sum_{j = 1}^{N} u_{j} a_{h}\left( \phi _{j}, \phi _{i} \right)  = l\left( \phi _{i} \right), \quad \forall
\]
...
To summarize the workflow of solving linear PDEs using the FEM method.

\begin{enumerate}[label=\arabic*)]
    \item Strong formulation: Find $u \in V$ such that  $ \mathcal{A} u = f   $.
    \item Abstract linear problem: Find $u \in V$ such that  $a( u,v) = l( v) \  \forall v \in V  $.
    \item Discrete linear problem: Find $u \in V_{h}$ such that  $a_{h}( u,v) = l( v) \  \forall v \in V_{h}  $.
...
Choose any triangle $T \in \mathcal{T}_{h} $ and let $v \in V_{h}  $. Then does the local inverse estimate hold,
$$
