...
\end{assumption}
\todo[inline]{ This assumption may in fact not be correct since we assume in CutDG to have a $\Gamma $ in $C^2$.   }
...
\begin{definition}[Mesh]
    ...
    \]
    We say that each $T \in  \mathcal{T} $ is a mesh element or simply as a element. We also denote $\mathcal{T} _{h}$ to be a mesh $\mathcal{T} $ with size $h$.
\end{definition}
