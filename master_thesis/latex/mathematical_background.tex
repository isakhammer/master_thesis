...
\]
Hence, the finite element $( T, \mathcal{P}, \Sigma  ) $ is reconstructed via the reference finite element $( \hat{T}, \hat{\mathcal{P} }, \hat{\Sigma} ) $ associated with the notes $\left\{ a_{i} \right\}_{i\in N} $.
...
Hence, affine meshes is essential for the error analysis which utilize the interpolation estimates, but it limits us to work on structure mesh if we work on quadrilatural meshes.
