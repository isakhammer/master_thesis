...
\begin{definition}[Local polynomial space]
    ...
    Let $T$ be a cuboid, i.e.,  $T = \prod_{i=1}^{d} [z_{i}^{-},z_{i}^{+}]$ where $z_{i}^{-}< z_{i}^{+}$ for $z_{i}^{\pm} \in \mathbb{R} $. Then do we define the polynomial space $\mathcal{Q}^{k}( T)$  in $\mathbb{R} ^{d}$ as the tensor product of 1-dimensional
    finite elements, i.e.,
      \[
...
We may introduce the reference element $\hat{T} $ in $d$ dimensions. The reference for a quadrilatural is denoted as $\hat{T} = [0,1]^{d}$. The reference for a simplex in  is defined by the convex hull spanned by the points $( z_{0}, e_{1}, \ldots, e_{d}) $ where $z_{0}:=0$ is the origin and $ \left\{ e_{i}
\right\}_{i=1}^{d} $ is the standard Cartesian basis in $ \mathbb{R} ^{d} $. A corresponding reference finite element is defined as $( \hat{T}, \hat{\mathcal{P} }, \hat{\Sigma}   ) $. Let the mapping $\mathcal{G} : \hat{T} \to  T$ as the affine
...
\begin{figure}[hb!]
    ...
    \subfloat[]{
        \begin{tikzpicture}[scale=1.0]
            ...
            % Draw arrow to signify the mapping G
            \draw[->, thick, >=stealth] ($(G1)+(0.1,0.1)$) to[bend left] node[midway, above, yshift=0.1cm] {$\mathcal{G}$} ($(G2)+(-0.2,0.2)$);
        \end{tikzpicture}
    }
    \caption{Illustration of affine mapping $\mathcal{G} : \hat{T} \to T $ }
\end{figure}
