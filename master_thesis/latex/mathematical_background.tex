...
Recall, the "$\lesssim$" symbol often denotes an inequality up to a constant factor. That is, given $a,b >0 $, the statement $a \lesssim b$ is true if there exists a constant $C>0$ such that $a \leq Cb$. Generally, this constant will contain
information related to the properties of the mesh, such as shape regularity and quasi-uniformity, but it often also includes the maximum finite number or measure of a quantity. For instance, let  $a , b_{i}> 0 \text{ for } i = 1,\ldots,N  $ and  $a =  \sum_{i}^{N} b_{i}$, then is $ a
\lesssim \mathrm{max}_{i} \  b_{i}  $. To maintain clarity and avoid unnecessary complexity, we will not delve into this particular detail related to the constant.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Let $v \in  H^{m}( \Omega ) $, then is
        \begin{equation}
            \| D^{r}v \|_{\Omega   }^{  }  \lesssim \| v \|_{m,\Omega   }^{  }.
        \end{equation}
