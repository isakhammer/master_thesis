...
\begin{definition}[Broken polynomial spaces]
    Let $\mathcal{T}_{h} $ be a mesh of $\Omega \in \mathbb{R} ^{d} $. Let $\mathcal{P}^{k}(T) $ be the space of all polynomials of order $k$ in the mesh element $T$ in $\mathcal{T}_{h}$ . We define the $C^{0}$ continuous broken polynomial space as \[
    \mathcal{P}^{k} ( \mathcal{T}_{h} ) := \left\{ v \in C^{0}( \Omega )   \mid  v|_{T} \in \mathcal{P}^k( T) \quad  \forall T \in  \mathcal{T}_{h}   \right\}.
    \]
...
\begin{definition}[Local polynomial space]
    \label{def:label}
    Let $T$ be a element in a mesh $\mathcal{T}_{h} $,  $x = \left[ x_{1}, \ldots, x_{d} \right] ^{T} $ be a vector, and $\alpha  = \left[ \alpha _{1}, \ldots, \alpha _{d} \right] $ be a multi index.
...
The definition of a finite element is defined as the triplet $( \mathcal{P}, \mathcal{P}', T    ) $.
As expected, the polynomial space $\mathcal{P}^{k}( T)  $ has different properties regarding of $T$ is a simplicial element or quadrilateral element, respectively denoted as $P^{k}(T )  $ or $Q^{k}( T) $.
...
\begin{figure}[htb!]
    ...
    \subfloat[ $\mathcal{Q} _{3}( T)  $]{
        ...
        \caption{Illustration of the nodes for the element of either a simplex or a quadrilateral for $d=1$ .}
        \label{fig:ill_nodes}
\end{figure}
