...
We may introduce the reference element $\hat{T} $. The reference for a quadrilatural $\hat{T} = [0,1]^{d}$. However, for a unit $d$-simplex is defined by the convex hull spanned by the points $( z_{0}, e_{1}, \ldots, e_{d}) $ where $z_{0}:=0$ is the origin and $ \left\{ e_{i}
\right\}_{i=1}^{d} $ is the standard Cartesian basis in $ \mathbb{R} ^{d} $. A corresponding reference finite element is $( \hat{T}, \hat{\mathcal{P} }, \hat{\Sigma}   ) $.
