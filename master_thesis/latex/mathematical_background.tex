...
workaround for proving nonaffine local interpolation estimates, but the necessarrly key assumptions on the relationship between the nodes $a_{i}$ and the regularity of mapping $\mathcal{G} $ \cite[Chapter 13]{ErnGuermond2021}.
...
Let $v \in H^{s}( \Omega ) $ and $r = \mathrm{min}(s , k+1) $, then we have the following estimates,
\begin{equation}
    \label{eq:clement_estimates}
    \begin{split}
    \| v - C _{h}^{} v \|_{  l, \mathcal{T} _{h} }^{  } & \lesssim h^{r-l}\sum_{T \in \mathcal{T}_h} \| v \|_{ r, \omega(T) }^{  } \lesssim  h^{r-l-\frac{1}{2}}  \| v \|_{ r, \mathcal{T}_h  }^{  }, \quad 0\le l\le r, \\
    \| v - C ^{}_{h}v \|_{ l,\mathcal{F} _{h} }^{  } & \lesssim h^{r-l-\frac{1}{2}}\sum_{T \in \mathcal{T}_h} \| v \|_{ r, \omega(F)  }^{  } \lesssim h^{r-l-\frac{1}{2}} \| v \|_{ r, \mathcal{T}_h  }^{  }, \quad 0  \le  l \le   r-\frac{1}{2}, \\
    \| v - C ^{}_{h}v \|_{ l, \Gamma }^{  } & \lesssim h^{r-l-\frac{1}{2}} \sum_{T \in \mathcal{T}_h}  \| v \|_{ r,  \omega(T)  }^{  }\lesssim h^{r-l-\frac{1}{2}} \lesssim    \| v \|_{ r,  \mathcal{T}_h  }^{  }, \quad 0  \le  l \le  r-\frac{1}{2}.
    \end{split}
\end{equation}
...
Another very useful inequality is the so-called trace inequality which connect the relationship of evaluating the norm on element $ T $ and with any of the corresponding facets $F \in \partial T$. The general form is $\| v_{h} \|_{F   }^{  }  \lesssim
