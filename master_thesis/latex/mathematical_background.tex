...
Assume that we have a problem that satisfies the Lax-Milgram theorem and let $X _{h} \subseteq  X $ be the discrete polynomial space. We transition the problem to find a solution, $u_{h} \in  \mathcal{V}_{h}$, so it holds that $a\left( u_{h},v_{h} \right)  = l\left( v_{h} \right) \ \  \forall v_{h} \in X _{h} $.
...
\| u - u_{h} \|_{ X  }^{  }  \le  \frac{M}{\alpha } \inf_{v_{h} \in X_{h} } \|  v_{h} - u \|_{X  }^{  }.
\]
A useful property is that for a conformal numerical method to converge can we now simply require \[
\lim_{h \to 0}  \inf_{v_{h} \in  X_{h}}  \| v - v_{h} \|_{ X  }^{  } = 0 \quad  \forall v \in \mathcal{V}.
\]
...
For more information, see \cite{quartdiff}.
