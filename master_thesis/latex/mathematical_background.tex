...
% A consequence of this is that classical methods are not able to approximate curved boundaries for quadrilatural elements unless nonaffine interpolation estimates is provided.
...
\end{figure}
 ...
 Recall the nodes  $\left\{ a_{i}\right\}_{i\in N}$ to be the Lagrange nodes on one or more elements in $\mathcal{T}_{h}$. Associated with each node $a_{i}$ we denote the macroelement $A_{i}$ to consist of all elements containing $a_{i}$. Let $n_{cf}$ be the number of configurations for the macroelement, then we define the index $j:
...
The Cléments interpolation operator $C_{h}$ is defined as a $L^2$-projection onto the macroelements. That is, given
