...
Hence, we now have a good motivation to quickly define the discrete $l^{p}$ norm, i.e.,\[
    \| U  \|_{ p }^{  } = \sum_{i=1}^{N}  \abs{ U_{i}  } \text{ for } U \in \mathbb{R} ^{N}
,
\]
where $1\le p < \infty$ and $\| U \|_{ \infty }^{  }  = \max_{i}  \abs{ U_i } $. We then define the matrix norm as \[
\| \mathcal{A}  \|_{ p }^{  }  = \max_{U \in \mathbb{R} ^{N} \setminus 0} \frac{\| AU \|_{ p }^{  } }{\| U \|_{ p }^{  } } \text{ for } \mathcal{A} \in \mathbb{R} ^{N\times N}.
\]
Assume that $\mathcal{A} $ is isomorphic, i.e. invertible, then we denote the condition number as \[
\kappa_{p} ( \mathcal{A} ) = \| \mathcal{A}  \|_{ p }^{  } \| \mathcal{A} ^{-1} \|_{ p }  .
\]
