...
\mathbf{u} =\mathbf{f} \).
Ultimately, the matrix $A$ is shown to be symmetric and positive definite only if $a_{h}( \cdot ,\cdot ) $ is symmetric and coercive, \cite[Theorem 4.1]{quartdiff}.
...
\begin{enumerate}[label=\arabic*)]
    ...
    \item Linear System: Find $u \in X_{h}$ such that  $ A \mathbf{u} = \mathbf{f}$.
\end{enumerate}
...
&= a\left( u - u_{h}, u -v_{h} \right) - a\left( u -u_{h}, v_{h} - u_{h} \right)  \\
 &  \le  M \| u - u_{h} \|_{ X  }^{  }  \| u - v_{h} \|_{ X  }^{  }
    \end{split}
.\end{equation}
Hence, we now have the so-called Ce's' lemma, \[
