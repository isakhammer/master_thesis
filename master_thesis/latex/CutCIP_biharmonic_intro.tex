...
\caption{An illustration of the background mesh $\widetilde{\mathcal{T} }_{h}$,  cut elements $\mathcal{T} _{\Gamma }$ and the interior of the active set $\mathcal{T} _{int}$.      }
\end{figure}
...
\begin{tikzpicture}
    ...
    \foreach \i in {0, 30, ..., 330} {
        \draw[fill=gray!30] (0, 0) -- (\i:1.5cm) -- (\i+30:1.5cm) -- cycle;
    }

    % Mesh nodes
    \foreach \i in {0, 30, ..., 330} {
        \fill[black] (\i:1.5cm) circle (1pt);
    }
...
\begin{tikzpicture}
    ...
    \draw (boundary-45) -- (boundary-90) -- (interior-1) -- cycle;
    \draw (boundary-90) -- (boundary-135) -- (interior-3) -- cycle;
    \draw (boundary-135) -- (boundary-180) -- (interior-3) -- cycle;
