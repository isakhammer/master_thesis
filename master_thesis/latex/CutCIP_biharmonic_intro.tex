...
$a_{h}:V_{h}
\times V_{h} \to  \mathbb{R}  $ and the linear form $l_{h}: V_{h} \to \mathbb{R} $. Recall the recall the Hessian formulation \eqref{eq:cip_biharmonic_form} and the Laplace formulation \eqref{eq:cip_biharmonic_form}. Then we have the following
unfitted variants.
...
\begin{enumerate}[label=\textbf{S.\arabic*}]
    ...
    \item \label{as:s3}For a $T \in \mathcal{T} _{\Gamma }$ there always exists a patch $\omega ( T) $ which contains $T$ and an element $T'$ with a so-called fat intersection $
        \abs{ T' \cap \Omega  } _{d} \gtrsim \abs{ T' } _{d}$, where $\abs{ \cdot  }_{d} $ is the measure of an element. For an illustration, see Figure \ref{fig:fat_intersection_property}.
\end{enumerate}
