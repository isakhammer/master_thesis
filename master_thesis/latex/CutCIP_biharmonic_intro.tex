...
\begin{figure}
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \fill[black] (interior-4) circle (2pt);
            ...
            \draw (boundary-225) -- (boundary-270) -- (interior-4) -- cycle;
            \draw (boundary-270) -- (boundary-315) -- (interior-4) -- cycle;
            ...
            \draw (interior-1) -- (interior-2) -- (interior-4) -- cycle;

            \draw[blue, thick] (-2.5, -2.5) rectangle (2.5, 2.5);

        \end{tikzpicture}
    \end{minipage}
\caption{Examples of different triangulations for a fitted mesh and a unfitted mesh for a circle.}
...
In contrast to the unfitted mesh in Figure \ref{fig:domain_mesh}, Figure \ref{fig:domain_mesh} shows examples of fitted meshes, where the mesh conforms to the domain boundary. However, generating such meshes for complex geometries can be cumbersome and time-consuming. CutFEM offers a more flexible approach, enabling us to work with standard meshes and focus on the PDEs' discretization and the imposition of boundary conditions.

Continuous interior penalty was first used for parabolic and elliptic problems \cite{douglas2008interior, arnold1982interior}. It was later applied to biharmonic problems.

For the CutFEM formulation we impose the Neumann boundary condition similarly to the Nitsche's method \cite{nitsche1971variationsprinzip}.
