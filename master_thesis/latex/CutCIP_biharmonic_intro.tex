...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.4\textwidth}
        ...
        \end{tikzpicture}
    \end{minipage}
        \caption{Two examples of bad cut cells. on the left side is $\abs{ \Gamma \cap T }_{d} \ll  h^{d-1}$ and correspondingly on the right side can we see that, $\abs{ T \cap \Omega  }_{d} \ll \abs{ T }_{d}  \lesssim h^{d}$. At some point is it even hard to distinguish between interior and cut cells.}
