...
\begin{figure}[t]
    ...
    \end{tikzpicture}
\caption{Illustration of the fat intersection property. Let $T \in \mathcal{T}_{\Gamma } $. Then is it clear that  $\abs{ \Gamma \cap T } \lesssim  h^{d-1}$ and $\abs{ \Omega  \cap T } \lesssim  \varepsilon  h^{d}$.  It shows a patch $\omega ( T) $ that contains elements $T \in  \mathcal{T}_{\Gamma } $ and $T'\in \mathcal{T} _{int} \cup \mathcal{T} _{\Gamma } = \mathcal{T} _{h}$, with $T' $ having a sufficiently large intersection with $\Omega$.}
