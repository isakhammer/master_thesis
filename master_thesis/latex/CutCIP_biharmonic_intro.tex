...
Let $\Omega \subseteq    \mathbb{R} ^d$ s.t. $\Gamma  $ be its corresponding boundary in $C^2$. Let the BH have the form,
\begin{equation}
...
$.
\begin{remark}
    Be aware that the only difference between the BH variants, \eqref{eq:bi_problem} and \eqref{eq:bi_main_problem}, is that the first one problem is fitted s.t. $\mathcal{T}_{h} = \Omega  $, while the latter is unfitted and equipped with a sufficiently smooth boundary.
\end{remark}
