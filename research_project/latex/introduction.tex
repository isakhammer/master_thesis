...
Ultimately will the cell membrane consists of interaction several kinds of physics (temperature, elasticity, chemical diffusion, internal fluid pressure etc.), hence, is it by interest to try to model the interaction. An example is to try to the
energy functionals \eqref{eq:GL}  and \eqref{eq:WE}, since the lipid-rafts formation is said to change the elasticity properties of the membrane, may be a good model for how cell membranes evolve to specific shapes or execute cell division. In fact,
some concluded that this process can be the background to cell fission. One way to couple the equations \eqref{eq:CH} and \eqref{eq:GL} is to let the parameters $b_{b}, b_{k}, H_{0} $  be some function of the concentration $c$ , i.e.,
\[
    \begin{split}
    ...
    \end{split}
\]
For more information, see \cite{elliott2010surface}. Recently have some authors have also couple diffusion processes and the so-called mean curvature energy \cite{burger2021interaction, elliott2022numerical}.
\todo[inline]{Area restriction and volume restriction, need to find it  }
