...
\]
\todo[inline]{ Need to establish the equivalence of this notation the following definition: Where $x = a\left( x \right)  + d\left( x \right) \nu \left( a\left( x \right)  \right) $, $\Gamma _{h} \subset U_{\delta } =  \left\{ y + s \nu \left( y \right)  \mid  \left\lvert s
    \right\rvert  < \delta \left( y \right), y \in  \Gamma    \right\} $ introduced in \cite[Equation 2.3]{dziuk2013finite}. Here is $a\left( x \right) $  the projection of $x$  onto $\Gamma $  and $d\left( x \right) $ is the distance function from $x $ and $\Gamma
$. Obviously is $\nu\left( a\left( x \right)  \right)   $ the normal vector on the projection point on $\Gamma $.}
...
\todo[inline]{ Formally in \cite[p 48]{sokolowski1992introduction} says that it might be an idea to formulate normal unit-vector regularity as $C^{\infty}$ }
...
$$
\nabla_{\Gamma
} u  = \nabla \widetilde{u} \mid _{\Gamma } - \left<\nu, \nabla \widetilde{u} \mid _{\Gamma } \right> \nu.
$$
Here $\widetilde{u}$ is a smooth extension of $u: \Gamma \to \mathbb{R} $ to an $3$-dim neighbourhood $U\in \mathbb{R} ^{3} $ of the surface $\Gamma $ so that $\widetilde{u} \mid _{\Gamma } = u$. For more information see \cite[ Definition 2.3]{dziuk2013finite}.
...
The method we will use in this paper to minimize the energy functional \eqref{eq:WE} is to compute the so-called gradient flow. The fundamental idea of the gradient flow is to give rise to evolutionary dynamics to decrease the overall energy
