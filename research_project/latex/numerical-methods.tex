...
\end{align}
\end{subequations}
Remark that $\mathbf{g}$ and $\mathbf{f}$ are heavily nonlinear and unfortunately quite hard to discretize properly. Another observation is that the inital condition of $\mathbf{u}$ and $\mathbf{x}$  is easy to compute using the inital closed surface $\Gamma _{0}$. On the other hand, inital conditions for $\mathbf{w}$ has to be evaluated using \eqref{eq:WE_strong_form_V} and \eqref{eq:WE_strong_form_z}.
